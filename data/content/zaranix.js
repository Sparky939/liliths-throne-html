...
(function () {
    ...
    LT.defineNode({
        ...
        applyPreParsingEffects: function () {
            if (typeof LT.ensureAmber === "function")
                LT.ensureAmber();
            if (typeof LT.ensureKatherine === "function")
                LT.ensureKatherine();
        },
        getContent: function () {
        ...
        getResponses: function () {
            ...
            if (LT.game.flags.zaranixMaidsHostile) {
                list.push(new LT.Response("Knock door", "Zaranix's maids will recognise you on sight, and won't let you in. You'll have to find another way to get inside.", null).disable("Zaranix's maids will recognise you on sight, and won't let you in."));
            }
            else if (!workTime()) {
                list.push(new LT.Response("Knock door", "Nobody will come to answer the door at such an unsociable time. You'll have to come back during the day, or find another way to get inside.", null).disable("Nobody will come to answer the door at such an unsociable time."));
            }
            else {
                list.push(new LT.Response("Knock door", "Knock on the door and wait for someone to answer.", "zaranix.knock", function () {
                    placeAmberOnStreet();
                    LT.game.flags.zaranixDiscoveredHome = true;
                    incrementKnock();
                }));
            }
            list.push(new LT.Response("Climb fence", "Climb over the garden's fence and sneak inside.", null).disable("The garden and the rest of Zaranix's house are not in this build."));
            if (LT.game.flags.zaranixKickedDownDoor) {
                list.push(new LT.Response("Kick down door", "After your last entrance, the front door has been reinforced. You're unable to enter like this again.", null).disable("After your last entrance, the front door has been reinforced."));
            }
            else {
                list.push(new LT.Response("Kick down door", "Kick down the front door.", "zaranix.kick", function () {
                    var a = LT.ensureAmber();
    ...
    LT.defineNode({
        ...
        getContent: function () {
            if (LT.game.flags.zaranixKnockedOnDoor)
                return xml("KNOCK_ON_DOOR_REPEAT");
        ...
        getResponses: function () {
            if (LT.game.flags.zaranixKnockedOnDoor) {
                ...
                if (knockCount() >= 4) {
                    list.push(new LT.Response("Enter", "It looks like your persistence has paid off!", null).disable("That audience is inside Zaranix's house, which is not in this build."));
                }
                return list;
            }
            return [
    ...
    LT.defineNode({
        ...
        getContent: function () {
            return LT.parse("<p>Amber staggers back, bracing herself against the wall as she lets out one last hateful curse, [amber.speech(You... " +
        ...
        getResponses: function () {
            return [
                ...
                LT.ResponseSex("Used", "Amber starts fucking you.", {
                    ...
                    startText: "<p>Amber takes a firm grasp of your hips, before roughly lifting your ass a little higher. The sharp slap of her hand across your right cheek causes you to let out a little cry, which is met by the maid's aggressive growl, [amber.speech(Squeal all you want bitch, <i>you're mine now!</i>)]</p>",
    ...
    LT.defineNode({
        ...
        chrome: { left: true, right: true },
        getContent: function () {
            return LT.parse("<p>You collapse to the floor, totally exhausted from Amber's rough treatment of you. With a final, scornful sneer, Amber stands up, [amber.speech(That's all bitches like you are good for! Now get the fuck out of this house!)]</p>" +
...
//# sourceMappingURL=zaranix.js.map