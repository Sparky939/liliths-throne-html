...
(function () {
    ...
    function stepper(title, id, value, decDisabled, incDisabled) {
        return ('<div class="date-col"><p style="width:100%;margin:0;padding:0;text-align:center;"><b>' +
            title +
    ...
    function birthdayBlock(p) {
        ...
        var b = p.birthday;
        return ('<div class="container-full-width"><p style="text-align:center;margin:0;padding:0;"><b>Birthday</b></p>' +
    ...
    function appearanceHtml() {
        ...
        return ("<p>By the time the taxi finally pulls up to the British Museum, you're already almost five minutes late. The whole reason you're visiting London is to attend your aunt Lily's opening evening for her new exhibition, and as you hurriedly pay the driver his fare and step out of the car, you hope that she hasn't started her speech yet.</p>" +
    ...
    function stepDate(p, field, act) {
        ...
        if (field === "day")
            ...
            d.setMonth(d.getMonth() + dir * amount);
        else
            p.setAge(p.getAgeValue() + dir * amount);
        if (field !== "age")
            p.birthday = d;
        LT.clampPlayerAge(p);
    }
    ...
    function handleAct(act) {
        var p = LT.game.player;
        if (act === "CHOOSE_GENDER_MALE")
            p.setGender(LT.Gender.MALE);
        else if (act === "CHOOSE_GENDER_FEMALE")
            p.setGender(LT.Gender.FEMALE);
        else if (act.indexOf("CHOOSE_FEM_") === 0)
            p.setFemininity(LT.Femininity[act.slice("CHOOSE_FEM_".length)]);
        else if (act.indexOf("SEXUAL_ORIENTATION_") === 0)
            p.orientation = LT.Orientation[act.slice("SEXUAL_ORIENTATION_".length)];
        else if (act.indexOf("PERSONALITY_TRAIT_") === 0)
            p.togglePersonality(act.slice("PERSONALITY_TRAIT_".length));
        else if (act.indexOf("START_MONTH_") === 0) {
            LT.game.startingMonth = Number(act.slice("START_MONTH_".length));
            LT.clampPlayerAge(p);
        }
        else if (act.indexOf("BIRTH_DAY_") === 0)
            stepDate(p, "day", act);
        else if (act.indexOf("BIRTH_MONTH_") === 0)
            stepDate(p, "month", act);
        else if (act.indexOf("AGE_") === 0)
            stepDate(p, "age", act);
        else
            return;
    ...
    LT.startNewGame = function () {
        ...
        if (typeof LT.ensureWeather === "function")
            LT.ensureWeather();
        if (LT.game.player) {
