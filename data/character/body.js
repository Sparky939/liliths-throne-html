...
(function () {
    ...
    LT.syncBodyFromCharacter = function (ch) {
        if (!ch)
            return ch;
        if (!ch.body && typeof LT.ensureBody === "function")
            LT.ensureBody(ch);
        var b = ch.body;
        if (!b)
            return ch;
        if (ch.heightCm != null)
            b.height = ch.heightCm;
        if (ch.femininityValue != null)
            b.femininity = ch.femininityValue;
        if (ch.bodySize && ch.bodySize.id)
            b.bodySize = ch.bodySize.id;
        if (ch.muscle && ch.muscle.id)
            b.muscle = ch.muscle.id;
        ...
        if (ch.lipSize && ch.lipSize.id && b.face)
            b.face.lipSize = ch.lipSize.id;
        if (b.face)
            b.face.lipsPuffy = !!ch.lipsPuffy;
        if (ch.eye && b.eye)
            b.eye.iris = ch.eye.id || ch.eye;
        ...
        if (ch.hair && b.hair)
            b.hair.colour = ch.hair.id || ch.hair;
        ...
        if (b.breast && b.breast.nipple)
            b.breast.nipple.puffy = !!ch.nipplesPuffy;
        if (ch.assSize && ch.assSize.id && b.ass)
            b.ass.size = ch.assSize.id;
        if (ch.hipSize && ch.hipSize.id && b.ass)
            b.ass.hipSize = ch.hipSize.id;
        if (b.ass)
            b.ass.bleached = !!ch.anusBleached;
        if (ch.penisLength != null && b.penis)
            b.penis.length = ch.penisLength;
    ...
    LT.applySavedBody = function (ch, data) {
        ...
        if (data) {
            if (data.fetishes)
                ch.fetishes = data.fetishes;
            if (data.fetishDesire)
                ch.fetishDesire = data.fetishDesire;
            if (data.perks)
                ch.perks = data.perks;
            if (data.perkPoints != null)
                ch.perkPoints = data.perkPoints;
            if (data.attributes)
                ch.attributes = data.attributes;
            if (data.affection)
                ch.affection = data.affection;
            if (data.obedience != null)
                ch.obedience = data.obedience;
            if (data.pregnancy)
                ch.pregnancy = data.pregnancy;
            if (data.offspring)
                ch.offspring = data.offspring;
            if (data.tattoos)
                ch.tattoos = data.tattoos;
            if (data.makeup)
                ch.makeup = data.makeup;
            if (data.scars)
                ch.scars = data.scars;
            if (data.piercings)
                ch.piercings = data.piercings;
            if (data.addictions)
                ch.addictions = data.addictions;
            if (data.potionAttributes)
                ch.potionAttributes = data.potionAttributes;
            if (data.enchantmentLimit != null)
                ch.enchantmentLimit = data.enchantmentLimit;
            if (data.companions)
                ch.companions = data.companions;
            if (data.sexCount)
                ch.sexCount = data.sexCount;
        }
