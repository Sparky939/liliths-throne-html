...
(function () {
    ...
    function bag(ch) {
        ...
        if (!ch.pregnancy.possibilities)
            ch.pregnancy.possibilities = [];
        if (!ch.offspring)
            ch.offspring = [];
        return ch.pregnancy;
    }
    ...
    LT.isVisiblyPregnant = function (ch) {
        ...
        if (!ch)
            return false;
        return (LT.hasStatusEffect(ch, "PREGNANT_1") ||
            LT.hasStatusEffect(ch, "PREGNANT_2") ||
            LT.hasStatusEffect(ch, "PREGNANT_3"));
    };
    LT.isAbleToBeImpregnated = function (ch) {
        if (!ch || !(ch.hasVagina && ch.hasVagina()))
            return false;
        if (LT.isVisiblyPregnant(ch))
            return false;
        if (ch.pregnancy && ch.pregnancy.incubating && ch.pregnancy.incubating.VAGINA)
            return false;
        return true;
    };
    LT.pregnancyChance = function (mother, father) {
        if (!LT.isAbleToBeImpregnated(mother))
            return 0;
        if (father && father.hasPenis && !father.hasPenis())
            return 0;
        var chance = 0.1;
    ...
    function chanceLabel(chance) {
        if (chance <= 0)
            return { id: "NO_CHANCE", name: "no chance" };
        if (chance < 0.2)
            return { id: "LOW", name: "small chance" };
        if (chance < 0.6)
            return { id: "AVERAGE", name: "chance" };
    ...
    function makeLitter(mother, father, count) {
        ...
        return {
            id: (mother.id || "pc") + "-" + ((mother.pregnancy && mother.pregnancy.littersGenerated) || 0),
    ...
    LT.rollForPregnancy = function (mother, father) {
        ...
        var html = "<p class='centre noPad'>As " +
        ...
        if (!LT.isPregnant(mother)) {
            if (!LT.hasStatusEffect(mother, "PREGNANT_0")) {
                var hours = PREG_0_MIN_HOURS + Math.floor(Math.random() * PREG_0_EXTRA_HOURS);
    ...
    LT.lastLitterBirthed = function (ch) {
        ch = ch || (LT.game && LT.game.player);
        if (!ch || !ch.offspring || !ch.offspring.length)
            return null;
    ...
    function startFirstPregnancyQuest() {
        ...
        if (!flags)
            return;
        if (flags.pregnancyQuest === "complete")
            return;
    ...
    LT.applyPregnancyStageExpire = function (ch, id) {
        ...
        if (id === "PREGNANT_0") {
            if (LT.isPregnant(ch)) {
                ...
                return ("<p>For the last few hours, your belly has been gradually swelling. The progress was so slow that you didn't even realise anything was happening, but as you glance down at your stomach, there's no mistaking it. You're pregnant.</p>" +
            ...
            if (!ch.player)
                return "";
            return ("<p>Enough time has passed now for you to be sure that you're in the clear. There's no sign of any bump in your belly, and you realise that despite having unprotected sex, you managed to avoid getting pregnant.</p>" +
    ...
    function consultHtml(first) {
        if (!first)
            return birthXml("LILAYA_ASSISTS_PREGNANCY_REPEAT");
        ...
        if (hadSex && p && p.pregnancy && p.pregnancy.possibilities) {
            ...
            if (anyLilaya && anyOther)
                html += birthXml("LILAYA_ASSISTS_PREGNANCY_LILAYA_POSSIBLY_FATHER");
            else if (anyLilaya)
                html += birthXml("LILAYA_ASSISTS_PREGNANCY_LILAYA_DEFINITELY_FATHER");
            else
                html += birthXml("LILAYA_ASSISTS_PREGNANCY_LILAYA_DEFINITELY_NOT_FATHER");
        }
    ...
    function birthResponses() {
        ...
        if (!ready)
            give.disable("You need to wait until your belly has finished growing before you're able to give birth.");
        return [
            new LT.Response("Back", "Tell Lilaya that you need a moment to think.", "lab.entry"),
            give,
        ];
    }
    LT.defineNode({
    ...
    LT.defineNode({
        ...
        getContent: function () {
            ...
            return (birthXml("LILAYA_ASSISTS_BIRTHING_DELIVERS") +
    ...
    LT.defineNode({
        ...
        getContent: function () {
            var litter = LT.lastLitterBirthed();
            var count = (litter && litter.count) || 1;
            return (birthXml("LILAYA_ASSISTS_BIRTHING_FINISHED") +
