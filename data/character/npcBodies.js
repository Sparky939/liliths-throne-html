...
(function () {
    ...
    LT.applyOfficialBody = function (ch, preset) {
        ...
        if (preset.ageAppearance)
            ch.ageAppearance = preset.ageAppearance;
        if (preset.raceConcealed)
            ch.raceConcealed = true;
        if (preset.description)
            ch.description = preset.description;
        if (preset.subspecies) {
            ch.raceName = preset.subspecies;
            ch.fullRace = preset.subspecies;
        }
        if (preset.makeup) {
            // ch.makeup's honest `boolean | Record<...>` type (see ItemOwner's own
            // comment on the field) needs narrowing past the boolean case before it
            // can be indexed — same pattern as player.ts's equivalent narrowing.
            if (!ch.makeup || typeof ch.makeup !== "object")
                ch.makeup = {};
            var makeupRecord = ch.makeup;
            var presetMakeup = preset.makeup;
            Object.keys(presetMakeup).forEach(function (slot) {
                makeupRecord[slot] = { colour: presetMakeup[slot], modifier: "MAKEUP" };
            });
        }
...
//# sourceMappingURL=npcBodies.js.map