...
(function () {
    ...
    LT.setMoveCooldown = function (ch, id, turns) {
        ...
        if (turns > 0)
            ch.moveCooldowns[id] = turns;
        else
            delete ch.moveCooldowns[id];
    };
    LT.lowerMoveCooldowns = function (ch) {
        ...
        for (id in ch.moveCooldowns) {
            if (!Object.prototype.hasOwnProperty.call(ch.moveCooldowns, id))
                continue;
            ch.moveCooldowns[id] -= 1;
            if (ch.moveCooldowns[id] <= 0)
                delete ch.moveCooldowns[id];
        }
    };
    ...
    function registerMagDump(def) {
        LT.MOVES[def.id] = {
            ...
            special: true,
            canUse: function (src) {
                if (!weaponNamed(src, def.weaponId))
                    return false;
                return LT.getMoveCooldown(src, def.id) <= 0;
            },
            cannotUseReason: function (src) {
                if (!weaponNamed(src, def.weaponId))
                    return def.needReason;
                var cd = LT.getMoveCooldown(src, def.id);
                if (cd > 0)
                    return "This action can't be used since it is still on cooldown! " + cd + " turns remaining.";
            ...
            perform: function (src, tgt) {
                ...
                if (tgt && typeof LT.applyTypedDamage === "function")
                    LT.applyTypedDamage(tgt, dmg, "PHYSICAL");
                else if (tgt)
    ...
    LT.WEAPON_SPECIALS = {
        MKAR_MAG_DUMP: {
            ...
            tooltip: "Switch to full-auto and empty an entire 30-round magazine at your enemies, dealing 21000 damage for each bullet that hits. Costs 2 AP. Cooldown 2 turns.",
            flavour: "Pushing the fire selector on [npc.her] {weapon} up into fully automatic, [npc.name] [npc.verb(take)] aim and [npc.verb(pull)] the trigger, unleashing a deafening, deadly hail of bullets at [npc2.name]!",
        },
        BR14_MAG_DUMP: {
            id: "BR14_MAG_DUMP",
            name: "mag dump",
            ...
            tooltip: "Switch to full-auto and empty an entire 20-round magazine at your enemies, dealing 26000 damage for each bullet that hits. Costs 2 AP. Cooldown 2 turns.",
            flavour: "Flicking the fire selector on [npc.her] {weapon} into fully automatic, [npc.name] [npc.verb(take)] aim and [npc.verb(pull)] the trigger, unleashing a deafening, deadly hail of bullets at [npc2.name]!",
        },
        FAUXMAS_MAG_DUMP: {
            id: "FAUXMAS_MAG_DUMP",
            name: "mag dump",
            ...
            tooltip: "Switch to full-auto and empty an entire 25-round magazine at your enemies, dealing 18000 damage for each bullet that hits. Costs 2 AP. Cooldown 2 turns.",
            flavour: "Pushing the fire selector on [npc.her] {weapon} into fully automatic, [npc.name] [npc.verb(take)] aim and [npc.verb(pull)] the trigger, unleashing a deafening, deadly hail of bullets at [npc2.name]!",
        },
    };
    ...
    LT.availableSpecials = function (ch) {
        ...
        for (var i = 0; i < LT.WEAPON_SPECIAL_IDS.length; i++) {
            ...
            if (weaponNamed(ch, def.weaponId))
                out.push(id);
        }
        return out;
    };
})();
//# sourceMappingURL=weaponSpecials.js.map