...
(function () {
    ...
    LT.getLootMoney = function (enemy) {
        if (enemy && enemy.lootMoney != null)
            return enemy.lootMoney;
    ...
    LT.getLootEssenceDrops = function (enemy) {
        if (enemy && enemy.lootEssences != null)
            return enemy.lootEssences;
        if (enemy && enemy.id === "brax")
            return 8;
        return 1 + Math.floor(Math.random() * enemyLevel(enemy));
    };
    LT.getLootItemId = function (enemy) {
        if (enemy && enemy.lootItems === null)
            return null;
        if (enemy && enemy.lootItems && !enemy.lootItems.length)
            return null;
        if (enemy && enemy.id === "brax")
            return null;
        var rnd = Math.random();
        if (rnd <= 0.05 && LT.itemType && LT.itemType("FETISH_UNREFINED"))
            return "FETISH_UNREFINED";
        if (rnd <= 0.1 && LT.itemType && LT.itemType("ADDICTION_REMOVAL"))
            return "ADDICTION_REMOVAL";
        ...
        if (tf)
            return tf;
        if (LT.itemType && LT.itemType("DYE_BRUSH"))
            return "DYE_BRUSH";
        return null;
    };
    LT.applyCombatVictoryLoot = function (enemy) {
        ...
        if (xp && typeof LT.incrementExperience === "function")
            html += LT.incrementExperience(xp);
        var money = LT.getLootMoney(enemy);
        if (money && typeof LT.incrementMoney === "function")
            html += LT.incrementMoney(money);
