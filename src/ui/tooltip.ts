(function () {
  ...
  function positionTooltip(tip: HTMLElement, x: number, y: number) {
    var pad = 16;
  ...
  LT.showTooltip = function (html: string, x: number, y: number) {
  ...
  LT.bindTooltip = function (el: Element, htmlOrFn: string | (() => string)) {
    el.addEventListener("mouseenter", function (e: Event) {
      ...
      if (html) LT.showTooltip(html, mouseEvent.clientX, mouseEvent.clientY);
    });
