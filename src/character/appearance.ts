(function () {
  ...
  function cap(s: unknown): string {
    s = String(s || "");
    return s ? (s as string).charAt(0).toUpperCase() + (s as string).slice(1) : "";
  }
  ...
  // list/id are genuinely polymorphic here — every enum-catalog in the
  // codebase (arrays like LT.SIZE5 and Records like LT.PART_TYPE) gets
  // passed through this same pair of helpers, and id can be a bare enum-id
  // string or the enum entry object itself. Same "not worth a union of
  // every enum type" reasoning as items/tfApply.ts's listOf.
  function named(list: any, id: any, fallback?: string): string {
    if (!id) return fallback || "";
    if (typeof id === "object") return id.name || fallback || "";
    if (list) {
      if (list[id] && list[id].name) return list[id].name;
      if (typeof LT.findById === "function") {
        var hit: any = LT.findById(list, id);
  ...
  function hexOf(list: any, id: any, fallback?: string): string {
  ...
  function colourSpan(list: any, id: any, text?: string): string {
    return "<span style='color:" + hexOf(list, id, "#ddd") + ";'>" + esc(text || named(list, id)) + "</span>";
  }

  function isPlayer(ch: CharOrNpc | null | undefined): boolean {
    return !!(ch && ((ch as Character).player || ch.id === "player" || (ch.isPlayer && ch.isPlayer())));
  }

  function feminine(ch: CharOrNpc): boolean {
    if (ch.isFeminine) return !!ch.isFeminine();
    return (ch.femininityValue || 50) >= 50;
  }

  function voice(ch: CharOrNpc): VoicePronouns {
    var you = isPlayer(ch);
    var fem = feminine(ch);
    if (you) {
      return {
    ...
    var name = ch.getName ? ch.getName() : ch.name || "They";
    return {
      name: name,
      Name: name,
      she: fem ? "she" : "he",
  ...
  function partName(type: string | null | undefined): string {
    if (!type || type === "NONE") return "";
    if (type === "HUMAN") return "human";
    return named(LT.PART_TYPE, type, String(type).toLowerCase().replace(/_/g, "-"));
  }

  function heightLabel(cm: number): { name: string; colour: string } {
    var n = Number(cm) || 170;
  ...
  function heightText(cm: number): string {
    var n = Number(cm) || 170;
  ...
  function ageCategory(age: number): AgeCategoryEntry {
    var list = LT.AGE_CATEGORY || [];
    var i;
    for (i = 0; i < list.length; i++) {
      if (age >= list[i].min && age <= list[i].max) return list[i];
    }
    return list[list.length - 1] || { id: "ADULT", name: "adult", colour: "#ddd", min: 0, max: 200 };
  }

  function makeupColour(ch: CharOrNpc, slotId: string): string {
    var makeup = ch.makeup;
    var rec = makeup && typeof makeup === "object" ? makeup[slotId] : null;
    if (!rec || !rec.colour || rec.colour === "NONE") return "";
    return named(LT.MAKEUP_COLOURS, rec.colour, rec.colour.toLowerCase().replace(/_/g, " "));
  }

  function hairStylePhrase(styleId: string): string {
  ...
  function bodyHairPhrase(level: string | null | undefined, area: string): string {
  ...
  function overview(ch: CharOrNpc, v: VoicePronouns, b: Partial<CharacterBody>): string {
    ...
    if (typeof LT.hasProperty !== "function" || LT.hasProperty("ageContent")) {
      // b.ageAppearance: CharacterBody has no such field (only Character
      // does) — LT.createBody/ensureExtras never set one, so this branch of
      // the original JS is always undefined too; kept as a permissive `any`
      // read rather than removed, matching the source exactly.
      var age = ch.getAgeValue
        ? ch.getAgeValue()
        : ch.ageAppearance || (b && (b as any).ageAppearance) || 25;
  ...
  function faceSection(ch: CharOrNpc, v: VoicePronouns, b: Partial<CharacterBody>): string {
  ...
  function mouthSection(ch: CharOrNpc, v: VoicePronouns, b: Partial<CharacterBody>): string {
  ...
  function torsoSection(ch: CharOrNpc, v: VoicePronouns, b: Partial<CharacterBody>): string {
    var html = header("Torso");
    var size = ch.bodySize || (LT.BODY_SIZE && LT.BODY_SIZE[b.bodySize as string]);
    var muscle = ch.muscle || (LT.MUSCLE && LT.MUSCLE[b.muscle as string]);
  ...
  function chestSection(ch: CharOrNpc, v: VoicePronouns, b: Partial<CharacterBody>): string {
  ...
  function crotchSection(ch: CharOrNpc, v: VoicePronouns, b: Partial<CharacterBody>): string {
  ...
  function armsSection(ch: CharOrNpc, v: VoicePronouns, b: Partial<CharacterBody>): string {
    var html = header("Arms");
    var rows = (b.arm && b.arm.rows) || 1;
    var type = partName((b.arm && b.arm.type) || "HUMAN");
    html += v.SheHas + " " + (rows === 1 ? "a pair" : rows + " pairs") + " of " + esc(type) + " arms.";
  ...
  function legsSection(ch: CharOrNpc, v: VoicePronouns, b: Partial<CharacterBody>): string {
    ...
    html += v.SheHas + " a pair of " + esc(type) + " legs, in a " + esc(config) + " configuration, ending in " + esc(feet) + " feet.";
    var toes = makeupColour(ch, "MAKEUP_NAIL_POLISH_FEET");
    if (toes) html += " " + v.Her + " toenails have been painted " + toes + ".";
    return html + "</p>";
  }

  function extrasSection(ch: CharOrNpc, v: VoicePronouns, b: Partial<CharacterBody>): string {
    var html = "";
  ...
  function assSection(ch: CharOrNpc, v: VoicePronouns, b: Partial<CharacterBody>): string {
    var html = header("Ass");
  ...
  function penisSection(ch: CharOrNpc, v: VoicePronouns, b: Partial<CharacterBody>): string {
    ...
    if (LT.knowsArea(ch, "PENIS")) {
      ...
      html += v.SheHas + " " + article(girth) + ", " + len + "-cm " + esc(type) + " cock.";
  ...
  function vaginaSection(ch: CharOrNpc, v: VoicePronouns, b: Partial<CharacterBody>): string {
    ...
    if (LT.knowsArea(ch, "VAGINA")) {
      ...
      html += v.SheHas + " " + article(esc(type)) + " pussy, with " + labia + " labia and " + article(clit) + " clit.";
      if (orifice) {
        html +=
          " It is " +
  ...
  function tattooSection(ch: CharOrNpc, v: VoicePronouns): string {
    var tats = ch.tattoos || {};
    ...
    for (i = 0; i < keys.length; i++) {
      ...
      html +=
        ...
        cap(slot) +
        ":</span> " +
        esc((t && (t.name || t.type)) || "a tattoo") +
        (t && t.writing ? ", bearing the words: '" + esc(t.writing) + "'" : "") +
        ".";
    }
    return html + "</p>";
  }

  function pregnancySection(ch: CharOrNpc, v: VoicePronouns): string {
  ...
  LT.getBodyDescription = function (ch) {
    ...
    var v = voice(ch);
    if (ch.raceConcealed) {
      return header("Overview") + esc(v.Name) + " is wrapped in a concealing cloak. As " + v.her + " body is mostly concealed, you can't tell what " + v.she + " really looks like." + "</p>";
    }
  ...
  function birthdayString(ch: CharOrNpc): string {
    var birthday = (ch as Character).birthday;
    if (!birthday) return "";
    var d = birthday;
    ...
    var s = ["th", "st", "nd", "rd"];
    var vv = n % 100;
    var ord = n + (s[(vv - 20) % 10] || s[vv] || s[0]);
    return ord + " of " + months[d.getMonth()] + " " + d.getFullYear();
  }

  function personalityHtml(ch: CharOrNpc): string {
    var html = "<h6>Personality</h6><p>";
    var traits: BodyEnumEntry[] = [];
  ...
  function relationshipHtml(ch: CharOrNpc): string {
    if (isPlayer(ch)) return "";
    var v = voice(ch);
  ...
  function clothingHtml(ch: CharOrNpc): string {
    ...
    if (!keys.length) {
      ...
      keys.forEach(function (slot, i) {
        var item = slots[slot];
        var name = (item && (item.name || item.id)) || slot;
        if (i) html += "<br/>";
  ...
  function fetishHtml(ch: CharOrNpc): string {
    var fet = ch.fetishes || {};
  ...
  function statsHtml(ch: CharOrNpc): string {
  ...
  LT.getCharacterInformationScreen = function (ch, opts) {
    ...
    html += "<h6><span class='char-info-name' data-tip-char='" + esc(portraitId) + "'>" + esc(name) + "</span></h6><p>";
    ...
    if (born) {
      ...
      if (age) {
        var cat = ageCategory(age);
        // TODO(ts): genuine display bug in the actual release, ported as-is.
        // The `(cond ? "" : "").trim()` term is always "" either way (dead
        // code), and `cat.name.replace(/s$/, "")` mangles most category
        // names that end in a plural 's' — "twenties" -> "twentie",
        // "thirties" -> "thirtie", etc. — rather than the intended
        // "twenty"/"thirty". Only "late teens" -> "late teen" reads right.
        html +=
          ", and " +
          v.is +
