...
(function () {
    ...
    LT.SPELLS = {
        FIREBALL: {
            ...
            castPc: "Summoning a swirling vortex of arcane fire around your [pc.arm], you focus its raw power into a ball of roiling flames before launching it at [npc.name]!",
            castNpc: "Summoning a swirling vortex of arcane fire around [npc.her] [npc.arm], [npc.she] focuses its raw power into a ball of roiling flames before launching it directly at [npc2.name]!",
        },
        ICE_SHARD: {
            id: "ICE_SHARD",
            name: "Ice Shard",
            ...
            castPc: "Summoning a swirling vortex of water from the moisture in the air, you focus your energy on freezing it in place, creating a shard of ice that you then launch at [npc.name]!",
            castNpc: "Summoning a swirling vortex of water from the moisture in the air, [npc.name] focuses [npc.her] energy on freezing it in place, creating a shard of ice that [npc.she] then launches at [npc2.name]!",
        },
        SLAM: {
            id: "SLAM",
            name: "Slam",
    ...
    LT.knownSpells = function (ch) {
        if (ch && ch.knownSpells && ch.knownSpells.length)
            return ch.knownSpells.slice();
        return [];
    };
    LT.learnSpell = function (ch, id) {
        if (!ch || !id || !LT.SPELLS[id])
            return;
        ch.knownSpells = ch.knownSpells || [];
        if (ch.knownSpells.indexOf(id) < 0)
            ch.knownSpells.push(id);
    };
    ...
    function parseCast(spell, src, tgt) {
        ...
        if (!text)
            return nameOf(src) + " casts " + spell.name;
    ...
    function colourOf(spell) {
        if (spell.effect === "lust")
            return LT.Colour.ATTRIBUTE_LUST;
        if (spell.damageType === "POISON")
            return LT.Colour.GENERIC_MINOR_GOOD;
        if (spell.damageType === "FIRE")
            return LT.Colour.GENERIC_BAD;
        if (spell.damageType === "ICE")
            return LT.Colour.ATTRIBUTE_MANA;
    ...
    function register(spell) {
        ...
        LT.MOVES[moveId] = {
            ...
            perform: function (src, tgt, turnIndex) {
                ...
                if (spell.effect === "flash") {
                    if (tgt && typeof LT.applyStatus === "function")
                        LT.applyStatus(tgt, "FLASH", 1);
                    return flavour + " " + nameOf(tgt) + " is dazzled (−1 AP). (-" + spent + " aura)";
                }
                if (spell.effect === "poison") {
                    if (tgt && typeof LT.applyStatus === "function")
                        LT.applyStatus(tgt, "POISON_VAPOURS", 3);
                ...
                if (tgt && typeof LT.applyTypedDamage === "function")
                    LT.applyTypedDamage(tgt, dmg, spell.damageType || "PHYSICAL");
                else if (tgt)
                    tgt.health = Math.max(0, (tgt.health || 0) - dmg);
                return flavour + " for <b>" + dmg + "</b> damage. (-" + spent + " aura)" + (crit.crit ? " <b>Critical hit!</b>" : "");
            },
        };
    }
    for (var i = 0; i < LT.SPELL_IDS.length; i++)
        register(LT.SPELLS[LT.SPELL_IDS[i]]);
})();
