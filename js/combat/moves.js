...
(function () {
    ...
    function weaponOf(src, slot) {
        if (slot === "offhand")
            return typeof LT.getOffhandWeapon === "function" ? LT.getOffhandWeapon(src) : null;
    ...
    function applyStrike(src, tgt, slot, turnIndex) {
        ...
        if (tgt && typeof LT.applyTypedDamage === "function")
            LT.applyTypedDamage(tgt, dmg, dtype);
        else if (tgt)
            tgt.health = Math.max(0, (tgt.health || 0) - dmg);
        var flavour;
        if (wep && typeof LT.weaponHitText === "function")
            flavour = LT.weaponHitText(src, tgt, wep);
        else
            flavour = nameOf(src) + " strikes " + nameOf(tgt);
        if (wep && typeof LT.consumeOneShot === "function")
            LT.consumeOneShot(src, slot, wep);
        var extra = "";
        if (cost && wep)
            extra += " (−" + cost + " essence" + (cost === 1 ? "" : "s") + ")";
        if (crit.crit)
            extra += " <b>Critical hit!</b>";
        ...
        allout: {
            ...
            canUse: function (src) {
                ...
                if (typeof LT.canAffordWeapon === "function") {
                    if (!LT.canAffordWeapon(src, "main"))
                        return false;
                    if (weaponOf(src, "offhand") && !LT.canAffordWeapon(src, "offhand"))
                        return false;
                }
                return true;
            },
            ...
            perform: function (src, tgt, turnIndex) {
                ...
                if (typeof LT.isTwoHandedEquipped === "function" && LT.isTwoHandedEquipped(src))
                    return main;
