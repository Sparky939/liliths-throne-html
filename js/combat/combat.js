...
(function () {
    ...
    LT.combat.start = function (opts) {
        ...
        if (opts.behaviour)
            enemy.combatBehaviour = opts.behaviour;
        this.planEnemy();
    };
    ...
    LT.combat.typeWeight = function (type, src, tgt, already, rnd) {
        ...
        var mult = 1;
        if (behaviour === "ATTACK" && type === "ATTACK")
            mult = 10;
        if (behaviour === "DEFEND" && type === "DEFEND")
            mult = 10;
        if (behaviour === "SEDUCE" && type === "TEASE")
            mult = 10;
        if (behaviour === "SPELLS" && type === "SPELL")
            mult = 10;
        already = already || 0;
        if (type === "ATTACK") {
        ...
        if (type === "DEFEND") {
            if (healthPct(src) < 0.2)
                return 1.0 * mult + 0.5 * rnd() - 0.2 * already;
        ...
        if (type === "SPELL") {
            if (!this.affordableEnemySpell(src))
                return -999;
            return mult;
        }
        return 0;
    ...
    LT.combat.affordableEnemySpell = function (src) {
        ...
        for (var i = 0; i < ids.length; i++) {
            ...
            if (spell && move && typeof LT.canAffordSpell === "function" && LT.canAffordSpell(src, spell))
                return ids[i];
        }
        return null;
    };
    function queuedOfType(src, type) {
        ...
        for (var i = 0; i < moves.length; i++) {
            var id = moves[i].id;
            if (type === "ATTACK" && (id === "strike" || id === "offhand"))
                n++;
            else if (type === "DEFEND" && id === "block")
                n++;
            else if (type === "TEASE" && id === "tease")
                n++;
            else if (type === "SPELL" && String(id).indexOf("spell_") === 0)
                n++;
        }
        return n;
    }
    LT.combat.pickEnemyMove = function (src, tgt, used, rnd) {
        ...
        if (best === "ATTACK") {
            ...
            if (hasOff && !used.offhand)
                return { id: "offhand", type: "ATTACK" };
            return { id: "strike", type: "ATTACK" };
        }
        if (best === "DEFEND")
            return { id: "block", type: "DEFEND" };
        if (best === "TEASE")
            return { id: "tease", type: "TEASE" };
        if (best === "SPELL") {
            var sid = this.affordableEnemySpell(src);
            if (sid)
                return { id: "spell_" + sid, type: "SPELL" };
        }
        return { id: "strike", type: "ATTACK" };
    };
    LT.combat.planEnemy = function () {
        ...
        while ((src.remainingAP || 0) >= 1) {
            ...
            if (!move || (src.remainingAP || 0) < (move.ap || 1))
                break;
    ...
    LT.combat.canQueue = function (moveId) {
        ...
        if (!this.active || this.finished || !move || !this.player)
            return "You cannot use that now.";
        if ((this.player.remainingAP || 0) < move.ap)
    ...
    LT.combat.predictions = function (ch) {
        ...
        for (var i = 0; i < moves.length; i++) {
            ...
            if (move)
                list.push(move.predict(ch, moves[i].target));
        }
        return list;
    };
    LT.combat.endTurn = function () {
        ...
        if (!defeated(this.enemy) && !this.escaped)
    ...
    LT.combat.resolveCharacter = function (ch, lines) {
        ...
        for (var i = 0; i < moves.length; i++) {
            if (defeated(this.player) || defeated(this.enemy))
                break;
            var move = LT.MOVES[moves[i].id];
            if (!move)
                continue;
    ...
    LT.combat.finish = function () {
        ...
        if (result === "victory" && this.onVictory)
            extra += this.onVictory() || "";
        if (result === "defeat" && this.onDefeat)
            extra = this.onDefeat() || "";
        if (typeof LT.recoverThrownAfterCombat === "function")
            LT.recoverThrownAfterCombat();
        if (result === "escaped" && this.onEscape)
            this.onEscape();
        if (extra)
            LT.game.textEnd = extra;
        var dest = null;
        if (result === "victory")
            dest = this.victoryNode;
        else if (result === "defeat")
            dest = this.defeatNode || this.returnNode;
        else if (result === "escaped")
            dest = this.returnNode || this.defeatNode;
        if (dest)
            LT.game.setContent(dest);
        else if (this.returnNode)
            LT.game.setContent(this.returnNode);
    };
