...
(function () {
    ...
    LT.resistFromStatuses = function (ch, type) {
        ...
        if (typeof LT.getStatus === "function") {
            ...
            if (LT.getStatus(ch, "STONE_SHELL") && type === "PHYSICAL")
                n += 5;
            if (LT.getStatus(ch, "ARCANE_CLOUD") && type === "LUST")
                n += -25;
        }
        var b = ch && ch.enchantBonus;
        if (b) {
            if (type === "PHYSICAL")
                n += b.resistPhysical || 0;
            if (type === "LUST")
                n += b.resistLust || 0;
            if (type === "FIRE")
                n += b.resistFire || 0;
    ...
    LT.refreshShields = function (ch) {
        ...
        var i;
        for (i = 0; i < LT.SHIELD_TYPES.length; i++)
            ch.shields[LT.SHIELD_TYPES[i]] = 0;
    ...
    LT.applyTypedDamage = function (target, amount, type) {
        ...
        if (left > 0)
            target.health = Math.max(0, (target.health || 0) - left);
        return left;
    };
    LT.strikeDamageType = function (weapon) {
        if (weapon && weapon.damageType)
    ...
    LT.spellCostOf = function (ch, spell) {
        ...
        if (bonus)
            cost = Math.max(0, Math.round(cost * (1 - bonus / 100)));
        return cost;
    };
    LT.lustDamageBonus = function (ch) {
        var n = 0;
        if (typeof LT.getStatus === "function" && LT.getStatus(ch, "TELEPATHIC_COMMUNICATION"))
            n += 15;
        if (ch && ch.enchantBonus)
            n += ch.enchantBonus.damageLust || 0;
        return n;
    };
