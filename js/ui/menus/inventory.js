...
(function () {
    ...
    function weaponSlotRow(player, slot) {
        ...
        if (twoHanded) {
            ...
            return ('<div class="inv-slot"><div class="inv-slot-label">' +
                slot.label +
    ...
    function weaponPileItem(it) {
        ...
        var offBtn = type && type.twoHanded
            ? ""
            : '<span class="inv-wep-btn" data-inv-wep-off="' + it.uid + '">Offhand</span>';
        return ('<div class="inv-item" style="border-color:' +
            LT.weaponRarityColour(type && type.rarity) +
            ';"><b>' +
            it.name +
            '</b><br><span class="muted">' +
    ...
    function inventoryHtml() {
        ...
        if (!p)
            return "<p>You have no inventory.</p>";
        if (typeof LT.ensureWeaponSlots === "function")
            LT.ensureWeaponSlots(p);
        var weapons = "";
        var wepSlots = LT.WEAPON_SLOTS || [];
        for (var w = 0; w < wepSlots.length; w++)
            weapons += weaponSlotRow(p, wepSlots[w]);
        var equipped = "";
        for (var i = 0; i < LT.SLOTS.length; i++)
            equipped += slotRow(p, LT.SLOTS[i]);
        var pile = "";
        for (var k = 0; k < (p.weapons || []).length; k++)
            pile += weaponPileItem(p.weapons[k]);
        ...
        if (!pile)
            pile = '<p class="muted">Your bags are empty.</p>';
        return ("<p>You take a moment to check what you're wearing, and what you've stuffed into your bags.</p>" +
    ...
    LT.defineNode({
        ...
        getResponses: function () {
            ...
            if (typeof LT.isDevMode === "function" && LT.isDevMode()) {
                list.push(new LT.Response("Take all weapons", "DEV: add one of every official weapon type you don't already have.", "inventory.main", function () {
                    if (typeof LT.grantAllWeapons === "function")
                        LT.grantAllWeapons(LT.game.player);
                }));
            }
            return list;
        },
    ...
    document.addEventListener("click", function (e) {
        if (!LT.game.currentNode || LT.game.currentNode.id !== "inventory.main")
            return;
        var p = LT.game.player;
        if (!p)
            return;
        ...
        if (enchantFirst) {
            if (typeof LT.openEnchant === "function")
                LT.openEnchant(enchantFirst.getAttribute("data-inv-enchant"));
            return;
        }
        ...
        if (enchant) {
            if (typeof LT.openEnchant === "function")
                LT.openEnchant(enchant.getAttribute("data-inv-enchant"));
            return;
        }
