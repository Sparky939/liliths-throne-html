...
(function () {
        ...
        getResponses: function () {
            return [
                ...
                (function () {
                    ...
                    if (!LT.game.player)
                        r.disable("You need a character first.");
                    return r;
                })(),
                ...
                (function () {
                    ...
                    if (!LT.game.started || !LT.game.player)
                        r.disable("You need to be in the game first.");
                    if (LT.game.currentNode && LT.game.currentNode.travelDisabled)
                        r.disable("You can't wait here.");
                    return r;
                })(),
                (function () {
                    var owned = typeof LT.ownedSlaves === "function" ? LT.ownedSlaves() : [];
                    if (!owned.length)
                        return new LT.Response("Slaves", "You do not own any slaves.", null).disable("You do not own any slaves.");
                ...
                (function () {
                    ...
                    if (!LT.game.player)
                        ...
                        r.disable((typeof LT.getUnableToTransformDescription === "function" && LT.getUnableToTransformDescription(LT.game.player)) ||
    ...
    LT.defineNode({
        ...
        getContent: function () {
            function block(q) {
                return ("<details open><summary class='quest-title' style='color:" +
            ...
            if (preg)
                html += block(preg);
            if (!html)
                return "<p class='muted'>You don't have any active quests.</p>";
            return html;
        },
        getResponses: function () {
    ...
    function waitAndReturn(seconds, hour, text) {
        if (hour != null && typeof LT.waitUntilHour === "function")
            LT.waitUntilHour(hour);
        else
            LT.game.advanceTime(seconds || 0);
    ...
    LT.defineNode({
        ...
        getContent: function () {
            ...
            if (!p)
                return "<p>Nobody to photograph.</p>";
            return ("<p>You hold the phone out and take a picture of yourself.</p>" +
        ...
        getResponses: function () {
            return [
                new LT.Response("Back", "Return to the phone.", "phone.menu"),
                new LT.Response(typeof LT.getCharacterImage === "function" && LT.getCharacterImage("player") ? "Change image" : "Set image", "Attach a portrait from an image link.", "house.image", function () {
    ...
    LT.defineNode({
        ...
        getContent: function () {
            ...
            if (!worlds.length && window.grid && grid.gridName)
                worlds = [grid.gridName];
            if (!worlds.length)
                return "<p class='muted'>You haven't explored any maps yet.</p>";
        ...
        getResponses: function () {
            ...
            for (i = 0; i < ids.length && i < 12; i++) {
                (function (id) {
                    ...
                    if (npc && npc.getName)
                        label = npc.getName();
                    list.push(new LT.Response(label, "Set or change this portrait.", "house.image", function () {
