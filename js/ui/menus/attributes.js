...
(function () {
    ...
    function bar(label, percent, colour, icon, current, max) {
        ...
        return ('<div class="resource-row" data-tip="' +
            label +
            ": " +
            nums +
            '">' +
    ...
    LT.paintAttributes = function () {
        ...
        if (typeof LT.paintStatusEffects === "function")
            LT.paintStatusEffects(player);
    };
    ...
    LT.initTimeListener = function () {
        document.addEventListener("lt-time", function () {
            ...
            if (el)
                el.textContent = LT.game.clock;
            var dateEl = document.getElementById("game-date");
            if (dateEl && typeof LT.formatGameDate === "function")
                dateEl.textContent = LT.formatGameDate();
            if (typeof LT.updateHouseNpcLocations === "function")
                LT.updateHouseNpcLocations();
            if (typeof LT.paintCharactersPresent === "function")
                LT.paintCharactersPresent();
            if (typeof LT.paintStatusEffects === "function")
                LT.paintStatusEffects();
        });
