...
(function () {
    ...
    function placeBody() {
        ...
        if (pt === "LILAYA_HOME_CORRIDOR") {
            return (p("Immaculately-clean red carpet runs down the centre of the wide corridor which you're currently walking down, while the walls are decorated in a pale, light-blue wallpaper that sports a series of delicate white floral patterns. Fine landscape paintings and portraits of beautiful demons are hung up on the walls at regular intervals, while all manner of antique curiosities are perched upon the many wooden cabinets which line the sides of these hallways.") +
                (isDay()
        ...
        if (pt === "LILAYA_HOME_STAIR_UP") {
            return (p("The grand, red-carpeted staircase is one of the first things you see when entering Lilaya's home. Half-way up the shallow steps, there's a secondary landing area, which is home to a couple of antique-looking cabinets and well looked after house plants. Branching off from the sides of this landing, two slightly-narrower staircases wrap around and lead up to the floor above.") +
        ...
        if (pt === "LILAYA_HOME_ROOM_ROSE") {
            ...
            return (p("Evidence of Rose's close relationship with Lilaya is apparent as you approach the cat-girl's room. Hanging on the door, there's a little home-made sign bearing her name, and underneath, in flowing handwriting which you recognise as Lilaya's, a little message reads: <i>'My loving Mistress'</i>") +
                (night
    ...
    function roomBody() {
        var html = placeBody();
        if (typeof LT.houseRoomContent === "function")
            return LT.houseRoomContent(html);
        return html;
    }
    function houseActions() {
        ...
        var pt = placeType();
        if (pt === "LILAYA_HOME_ROOM_PLAYER") {
            list.push(new LT.Response("Rest (1 hour)", "Rest for an hour. As well as replenishing your health and aura, you will also get the 'Well Rested' status effect.", null, function () {
                if (typeof LT.applySleepEffect === "function")
                ...
                if (delta <= 0)
                    delta += 86400;
                if (typeof LT.applySleepEffect === "function")
                    LT.applySleepEffect(LT.game.player, Math.round(delta / 60));
                if (typeof LT.waitUntilHour === "function")
                    LT.waitUntilHour(7);
                else
        ...
        if (typeof LT.houseExtraResponses === "function")
            return LT.houseExtraResponses(list);
        return list;
    }
    function fastTravel() {
        var here = placeType();
        function go(title, tip, world, place) {
            if (here === place)
                return new LT.Response(title, tip, null).disable("You are already here.");
    ...
    for (var i = 0; i < housePlaces.length; i++)
        definePlaceNode("place." + housePlaces[i], housePlaces[i]);
    LT.defineNode({
        id: "lilaya.room-stub",
        ui: "dialogue",
        title: "Your Room",
