...
(function () {
    ...
    function actionColour(act) {
        ...
        if (act.id === "do_nothing")
            return LT.Colour.GENERIC_MINOR_BAD;
        if (act.id === "manage_clothing")
    ...
    LT.defineNode({
        ...
        getContent: function () {
            var s = LT.sex;
            if (!s || !s.player || !s.partner)
                return "<p>There is no sex scene here.</p>";
        ...
        getResponses: function (game, tabIndex) {
            ...
            if (s.finished) {
                return [
                    slot(1, new LT.Response("Continue", "The encounter is over.", null, function () {
                        s.finish();
            ...
            for (i = 0; i < actions.length; i++) {
                (function (act, index) {
                    var title = act.name;
                    if (title && title.indexOf("[") >= 0 && LT.sex.parseText)
                        title = LT.sex.parseText(title, s.player, s.partner);
...
//# sourceMappingURL=sexNodes.js.map