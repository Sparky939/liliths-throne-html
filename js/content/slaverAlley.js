...
(function () {
    ...
    LT.defineNode({
        ...
        chrome: { left: true, right: true },
        applyPreParsingEffects: function () {
            if (typeof LT.ensureScarlett === "function")
                LT.ensureScarlett();
            if (typeof LT.ensureHelena === "function")
                LT.ensureHelena();
        },
        getContent: function () {
            ...
            if (!(LT.isWorkTime && LT.isWorkTime()))
                return shopXml("SCARLETTS_SHOP_EXTERIOR_CLOSED");
        ...
        getResponses: function () {
            ...
            if (!(LT.isWorkTime && LT.isWorkTime())) {
                list.push(new LT.Response("Enter", "Scarlett's Shop is currently closed, and will re-open at six in the morning. You'll have to come back some time after then.", null).disable("Scarlett's Shop is currently closed, and will re-open at six in the morning."));
            }
    ...
    LT.defineNode({
        ...
        getContent: function () {
            if (LT.game.flags && LT.game.flags.quest === "MAIN_1_D_SLAVERY")
                return shopXml("SCARLETTS_SHOP_INTRO");
    ...
    LT.defineNode({
        ...
        applyPreParsingEffects: function () {
            ...
            if (h)
                h.location = { world: "SLAVER_ALLEY", place: "SLAVER_ALLEY_SCARLETTS_SHOP" };
            if (s)
                s.location = { world: "SLAVER_ALLEY", place: "SLAVER_ALLEY_SCARLETTS_SHOP" };
        },
        getContent: function () {
            if (LT.game.flags && LT.game.flags.quest === "MAIN_1_F_SCARLETTS_FATE")
                return shopXml("HELENAS_SHOP_INTRODUCTION");
            if (LT.game.flags && LT.game.flags.quest === "MAIN_1_G_SLAVERY")
                return shopXml("HELENAS_SHOP_OFFER_SCARLETT");
            return shopXml("HELENAS_SHOP_INTRODUCTION");
        },
        getResponses: function () {
            var q = LT.game.flags && LT.game.flags.quest;
            if (q === "MAIN_1_F_SCARLETTS_FATE") {
                return [
                    ...
                    new LT.Response("Offer to buy", "Offer to buy Scarlett from Helena.", "helena.forSale", function () {
                        if (LT.game.flags.punishedByHelena)
                            LT.game.flags.scarlettPrice = 10000;
                        else
                            LT.game.flags.scarlettPrice = 15000;
            ...
            if (q === "MAIN_1_G_SLAVERY")
                list.push(scarlettBuyResponse());
            return list;
        },
    });
    LT.defineNode({
    ...
    function scarlettBuyResponse() {
        ...
        if (!(LT.game.flags && LT.game.flags.hasSlaverLicense)) {
            return new LT.Response("Buy Scarlett (" + price + ")", "You need to obtain a slaver license from the Slavery Administration before you can buy Scarlett!", null).disable("You need to obtain a slaver license from the Slavery Administration before you can buy Scarlett!");
        }
        if (LT.getMoney() < price) {
            return new LT.Response("Buy Scarlett (" + price + ")", "You don't have enough money to buy Scarlett.", null).disable("You don't have enough money to buy Scarlett.");
        }
    ...
    function purchasedScarlett(kind) {
        return function () {
            ...
            if (typeof LT.generateZaranixTile === "function")
    ...
    LT.defineNode({
        ...
        applyPreParsingEffects: function () {
            if (typeof LT.addSpecialParse === "function")
                LT.addSpecialParse(String(LT.scarlettPrice()), true);
        },
        getContent: function () {
        ...
        getResponses: function () {
            return [
                ...
                new LT.Response("Keep her", "You decide to keep Scarlett as your slave.", "helena.keep", function () {
                    ...
                    if (typeof LT.ensureScarlett === "function")
                        LT.ensureScarlett();
                    if (typeof LT.takeOwnership === "function")
                        LT.takeOwnership(LT.game.npcs.scarlett);
                }),
    ...
    function adminLicenseResponses() {
        ...
        if (!(LT.game.flags && LT.game.flags.slaveryQuest)) {
            list.push(new LT.Response("Slaver license", "Ask Finch about obtaining a slaver license.", "admin.askLicense", function () {
                LT.game.flags.finchIntroduced = true;
        ...
        if (LT.game.flags.slaveryQuest === "SIDE_SLAVER_RECOMMENDATION_OBTAINED") {
            if (LT.getMoney() >= cost) {
                list.push(new LT.Response("Present letter (" + cost + ")", "Show Finch the letter of recommendation you obtained from Lilaya, and then pay " + cost + " flames to obtain a slaver license.", "admin.licenseObtained", function () {
                    LT.game.textEnd = LT.incrementMoney(-cost);
                }));
            }
            else {
                list.push(new LT.Response("Present letter (" + cost + ")", "You don't have enough money to buy a slaver license! You need at least " + cost + " flames.", null).disable("You don't have enough money to buy a slaver license! You need at least " + cost + " flames."));
            }
        }
        else {
            list.push(new LT.Response("Present letter (" + cost + ")", "You need to obtain a letter of recommendation from Lilaya first!", null).disable("You need to obtain a letter of recommendation from Lilaya first!"));
        }
        return list;
    }
    LT.defineNode({
        ...
        chrome: { left: true, right: true },
        applyPreParsingEffects: function () {
            if (typeof LT.ensureFinch === "function")
                LT.ensureFinch();
        },
        getContent: function () {
    ...
    LT.defineNode({
        ...
        chrome: { left: true, right: true },
        getContent: function () {
            return LT.itemShopHtml("finch", "<p>Finch shows you the slavery supplies he keeps behind the counter. Metal collars are what you'll need if you intend to bring alley criminals in.</p>");
        },
        getResponses: function () {
    ...
    LT.defineNode({
        ...
        getContent: function () {
            ...
            if (!wait.length)
                return "<p>There are no slaves waiting to be collected.</p>";
        ...
        getResponses: function () {
            ...
            for (i = 0; i < wait.length; i++) {
                (function (index) {
                    var rec = wait[index];
                    list.push(new LT.Response("Collect " + rec.name, "Take ownership of " + rec.name + ".", "finch.collect", function () {
...
//# sourceMappingURL=slaverAlley.js.map