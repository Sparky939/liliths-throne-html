...
(function () {
    ...
    function pills(title, help, items, currentId, prefix, hexKey) {
        var html = '<div class="container-full-width" style="text-align:center;"><p style="text-align:center;margin:0;padding:0;"><b>' +
            title +
            "</b></p>";
        if (help)
            html += '<p style="text-align:center;">' + help + "</p>";
    ...
    function stepper(title, act, label, decOff, incOff) {
        return ('<div class="container-full-width" style="text-align:center;"><p style="margin:0;padding:0;"><b>' +
            title +
    ...
    function stepList(list, current, dir) {
        var i = 0;
        for (; i < list.length; i++)
            if (list[i] === current || list[i].id === current.id)
                break;
        if (i >= list.length)
            i = 0;
    ...
    function hubHtml() {
        ...
        var named = p.surname && p.surname.length
            ? p.getName() + " " + p.surname
            : p.getName();
        var title = p.surname
            ...
            : p.isFeminine()
                ? "Miss"
                : "Sir";
        return ("<p><span class=\"speech\">\"It's " +
            named +
    ...
    function ensureAppearanceExtras(p) {
        ...
        (LT.PIERCING_TYPES || []).forEach(function (slot) {
            if (p.piercings[slot.id] == null)
                p.piercings[slot.id] = false;
        });
        if (!p.tattoos)
            p.tattoos = {};
        if (p.body) {
    ...
    function syncPiercingToBody(p, id, on) {
        if (!p.body)
            return;
        if (id === "ear")
            p.body.ear.pierced = on;
        else if (id === "lip")
            p.body.face.piercedLip = on;
        else if (id === "tongue")
            p.body.face.tongue.pierced = on;
        else if (id === "navel")
            p.body.piercedStomach = on;
        else if (id === "nose")
            p.body.face.piercedNose = on;
        else if (id === "nipple")
            p.body.breast.nipple.pierced = on;
        else if (id === "penis")
            p.body.penis.pierced = on;
        else if (id === "vagina")
            p.body.vagina.pierced = on;
    }
    function canPierce(p, slot) {
        if (slot.needs === "penis")
            return !!(p.hasPenis && p.hasPenis());
        if (slot.needs === "vagina")
            return !!(p.hasVagina && p.hasVagina());
        return true;
    }
    function tattooSlotBlocked(p, slot) {
        if (!p.body)
            return false;
        if (slot.needs === "horns")
            return !p.body.horn || p.body.horn.type === "NONE";
        if (slot.needs === "wings")
            return !p.body.wing || p.body.wing.type === "NONE";
        if (slot.needs === "tail")
            return !p.body.tail || p.body.tail.type === "NONE";
        if (slot.needs === "penis")
            return !(p.hasPenis && p.hasPenis());
        if (slot.needs === "vagina")
            return !(p.hasVagina && p.hasVagina());
    ...
    function tattoosHtml() {
        var p = LT.game.player;
        ensureAppearanceExtras(p);
        var html = '<div class="container-full-width" style="text-align:center;"><i>Later on in the game, you can get enchanted and glowing tattoos. For now, however, your tattoo choices are limited to more mundane options.</i></div>';
    ...
    function tattooAddHtml() {
        ...
        return ('<div class="container-full-width" style="text-align:center;"><p>Add a ' +
            slot.name +
            " tattoo.</p></div>" +
    ...
    function handleAct(act) {
        ...
        if (!p)
            return;
        if (act === "HEIGHT_DEC")
            p.heightCm = Math.max(140, p.heightCm - 1);
        else if (act === "HEIGHT_INC")
            p.heightCm = Math.min(210, p.heightCm + 1);
        else if (act === "PENIS_DEC")
            p.penisLength = Math.max(5, p.penisLength - 1);
        else if (act === "PENIS_INC")
            p.penisLength = Math.min(40, p.penisLength + 1);
        else if (act.indexOf("SKIN_") === 0)
            p.skin = LT.findById(LT.SKIN, act.slice(5));
        else if (act.indexOf("SIZE_") === 0)
            p.bodySize = LT.BODY_SIZE[act.slice(5)] || p.bodySize;
        else if (act.indexOf("MUSCLE_") === 0)
            p.muscle = LT.MUSCLE[act.slice(7)] || p.muscle;
        else if (act.indexOf("LIP_") === 0 && act.indexOf("LIPPUFF") !== 0)
            p.lipSize = LT.LIP[act.slice(4)] || p.lipSize;
        else if (act === "LIPPUFF_ON")
            p.lipsPuffy = true;
        else if (act === "LIPPUFF_OFF")
            p.lipsPuffy = false;
        else if (act.indexOf("EYE_") === 0)
            p.eye = LT.findById(LT.EYE, act.slice(4));
        else if (act.indexOf("HLEN_") === 0) {
            p.hairLength = LT.HAIR_LENGTH[act.slice(5)] || p.hairLength;
            var li = LT.hairLengthIndex(p.hairLength.id);
            if (p.hairStyle.minLength > li)
                p.hairStyle = LT.findById(LT.HAIR_STYLE, li === 0 ? "NONE" : "MESSY");
        }
        else if (act.indexOf("HSTYLE_") === 0)
            p.hairStyle = LT.findById(LT.HAIR_STYLE, act.slice(7));
        else if (act.indexOf("HCOLOUR_") === 0)
            p.hair = LT.findById(LT.HAIR_COLOUR, act.slice(8));
        else if (act.indexOf("CUP_") === 0)
            p.breastSize = LT.CUP[act.slice(4)] || p.breastSize;
        else if (act.indexOf("BSHAPE_") === 0)
            p.breastShape = LT.findById(LT.BREAST_SHAPE, act.slice(7));
        else if (act.indexOf("NIP_") === 0 && act.indexOf("NIPPUFF") !== 0)
            p.nippleSize = LT.findById(LT.SIZE5, act.slice(4));
        else if (act.indexOf("ARE_") === 0)
            p.areolaeSize = LT.findById(LT.SIZE5, act.slice(4));
        else if (act === "NIPPUFF_ON")
            p.nipplesPuffy = true;
        else if (act === "NIPPUFF_OFF")
            p.nipplesPuffy = false;
        else if (act.indexOf("ASS_") === 0)
            p.assSize = LT.findById(LT.SIZE5, act.slice(4));
        else if (act.indexOf("HIP_") === 0)
            p.hipSize = LT.findById(LT.SIZE5, act.slice(4));
        else if (act === "BLEACH_ON")
            p.anusBleached = true;
        else if (act === "BLEACH_OFF")
            p.anusBleached = false;
        else if (act.indexOf("BALLS_") === 0)
            p.testicleSize = LT.findById(LT.SIZE5, act.slice(6));
        else if (act.indexOf("CAP_") === 0)
            p.vaginaCapacity = LT.findById(LT.SIZE5, act.slice(4));
        else if (act.indexOf("LABIA_") === 0)
    ...
    LT.defineNode({
        ...
        getResponses: function () {
            ...
            return [
                ...
                new LT.Response("Continue", "Your clothes are a little messy after rushing here. Tidy yourself up before proceeding to the main stage.", "creation.wardrobe").withTime(30),
                ...
                new LT.Response(p && p.hasPenis() ? "Penis" : "Vagina", "Enter the customisation menu for your genitals.", "creation.genitals"),
    ...
    LT.defineNode({
        ...
        getResponses: function () {
            ...
            var apply = new LT.Response("Apply", "Add this tattoo.", "creation.tattoos", function () {
                ...
                if (input)
                    next.writing = String(input.value || "").trim();
                p.tattoos[slotId] = {
                    type: next.type,
    ...
    document.addEventListener("click", function (e) {
        ...
        if (!node || String(node.id).indexOf("creation.") !== 0)
            return;
        if (node.id === "creation.appearance" || node.id === "creation.name")
            return;
        handleAct(btn.getAttribute("data-act"));
    });
})();
//# sourceMappingURL=advancedAppearance.js.map