...
(function () {
    ...
    function lootHtml() {
        ...
        if (!npc)
            return "<p>There is nobody here to loot.</p>";
        ...
        if (!worn.length)
            html += '<div class="inv-item empty-slot"><span class="muted">stripped</span></div>';
        ...
        function wepCell(which, wep) {
            if (!wep)
                return "";
            return ('<div class="inv-item" data-loot-wep="' +
                which +
                '"><b>' +
                wep.name +
                '</b><br><span class="muted">' +
    ...
    LT.defineNode({
        ...
        getResponses: function () {
            ...
            if (!npc)
                return list;
            list.push(new LT.Response("Strip all", "Unequip everything they are wearing and take it.", "loot.npc", function () {
            ...
            list.push(new LT.Response("Take bag", "Take every loose item in their bag.", "loot.npc", function () {
                var taken = LT.takeAllNpcItems(npc);
                LT.game.textStart = taken.length
                    ? "<p>You empty their bag (" + taken.length + " item" + (taken.length === 1 ? "" : "s") + ").</p>"
                    : "<p>Their bag is empty.</p>";
            }));
            list.push(new LT.Response("Take weapons", "Disarm them and take their weapons.", "loot.npc", function () {
    ...
    document.addEventListener("click", function (e) {
        if (!LT.game.currentNode || LT.game.currentNode.id !== "loot.npc")
            return;
        var npc = typeof LT.currentLootNpc === "function" ? LT.currentLootNpc() : null;
        if (!npc)
            return;
...
//# sourceMappingURL=lootNpc.js.map