...
(function () {
    ...
    LT.defineNode({
        ...
        getContent: function () {
            var c = LT.combat;
            if (!c || !c.player || !c.enemy)
                return "<p>There is no fight here.</p>";
            ...
            if (yours.length)
                html += "<p><b>You:</b> " + yours.join(" ") + "</p>";
            else
                html += "<p><b>You:</b> <span class='muted'>No moves queued.</span></p>";
            if (theirs.length)
                html += "<p><b>" + (c.enemy.getName ? c.enemy.getName() : c.enemy.name) + ":</b> " + theirs.join(" ") + "</p>";
            html += "</div>";
            if (c.lastResolution) {
        ...
        getResponses: function (game, tabIndex) {
            ...
            if (c.finished === "victory") {
                return [
                    slot(1, new LT.Response("Victory", "You have won this fight!", null, function () {
                        c.finish();
            ...
            if (c.finished === "defeat") {
                return [
                    slot(1, new LT.Response("Defeat", "You have been defeated!", null, function () {
                        c.finish();
            ...
            if (c.finished === "escaped") {
                return [
                    slot(1, new LT.Response("Escaped!", "You got away!", null, function () {
                        c.finish();
                    }).withColour(LT.Colour.GENERIC_GOOD)),
                ];
            }
...
//# sourceMappingURL=combatNodes.js.map