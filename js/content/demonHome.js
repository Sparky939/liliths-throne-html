...
(function () {
    ...
    LT.advanceMainQuest = function (nextId) {
        ...
        if (typeof LT.syncQuestWorld === "function")
            LT.syncQuestWorld();
        var html = "<p style='text-align:center;'><b style='color:" +
    ...
    LT.generateArthurTile = function () {
        ...
        var tile = null;
        if (tower)
            tile = findDominionXY(tower.x - 2, tower.y - 1);
    ...
    LT.generateZaranixTile = function () {
        ...
        var tile = null;
        if (tower)
            tile = findDominionXY(tower.x + 1, tower.y - 2);
        if (!tile || !tile.location || tile.location.placeType !== "DOMINION_DEMON_HOME") {
            tile = findDominionTile("DOMINION_DEMON_HOME");
        }
        if (!tile || !tile.location)
    ...
    LT.syncQuestWorld = function () {
        if (LT.questReached("MAIN_1_B_DEMON_HOME"))
            LT.generateArthurTile();
        if (LT.questReached("MAIN_1_H_THE_GREAT_ESCAPE"))
            LT.generateZaranixTile();
        if (typeof LT.ensureFelicia === "function")
            LT.ensureFelicia();
    ...
    function streetResponses() {
        ...
        if (pt === "DOMINION_DEMON_HOME_ARTHUR" && LT.questReached("MAIN_1_B_DEMON_HOME")) {
            list.push(new LT.Response("Sawlty Towers", LT.game.flags.quest === "MAIN_1_B_DEMON_HOME"
                ? "Find Arthur's apartment in the building using the instructions Lilaya gave to you."
                : "Head over to the Sawlty Towers apartment building.", "demonHome.apartment"));
        }
        if (pt === "DOMINION_DEMON_HOME_ZARANIX" && LT.questReached("MAIN_1_H_THE_GREAT_ESCAPE")) {
            list.push(new LT.Response("Zaranix's Home", "A little way down the road from Arthur's apartment building stands the home of Zaranix; the demon that Scarlett told you about.", "zaranix.outside"));
        }
        return list;
    }
    ...
    LT.defineNode({
        ...
        chrome: { left: true, right: true },
        getContent: function () {
            return (p("A set of huge, iron gates have been built across the street here, separating the regular areas of Dominion from that known as 'Demon Home' beyond. Half a dozen elite demon Enforcers are stationed here, keeping a close eye on anyone who comes and goes.") +
                p("As you walk forwards to pass through the gates, you see one of these demonic guards staring at you. Ignoring their penetrating gaze, you stride forwards, breathing a sigh of relief as you get through to the other side without being stopped."));
        },
    ...
    LT.defineNode({
        ...
        chrome: { left: true, right: true },
        applyPreParsingEffects: function () {
            if (typeof LT.maybePlaceEncounter === "function")
                LT.maybePlaceEncounter();
        },
        getContent: function () {
            return p("Lilaya's home is more of a mansion than a town house, and, due to its impressive size, stands out as a particularly impressive building in this area.");
        },
        getResponses: function () {
            return LT.travelResponses ? LT.travelResponses() : [null];
        },
    ...
    LT.defineNode({
        ...
        getResponses: function () {
            ...
            if (LT.game.flags.quest === "MAIN_1_B_DEMON_HOME") {
                list.push(new LT.Response("Arthur's room", "Head up to Arthur's room.", "demonHome.arthursRoom", function () {
                ...
                list.push(new LT.Response(LT.parse("[felicia.Name]'s room"), LT.parse("Head up to [felicia.namePos] room."), "demonHome.feliciasRoom"));
            }
            return list;
        },
    });
...
//# sourceMappingURL=demonHome.js.map