...
(function () {
    ...
    function primariesFor() {
        if (isPotion())
            return LT.RACIAL_PRIMARIES || [];
        if (isWeapon())
            return ["CLOTHING_MAJOR_ATTRIBUTE", "CLOTHING_ATTRIBUTE"];
        return ["CLOTHING_MAJOR_ATTRIBUTE", "CLOTHING_ATTRIBUTE", "CLOTHING_SPECIAL"];
    }
    function secondariesFor(primary) {
        if (isPotion())
            return (LT.RACIAL_SECONDARIES && LT.RACIAL_SECONDARIES[primary]) || [];
        if (isWeapon()) {
            if (primary === "CLOTHING_ATTRIBUTE")
                return LT.WEAPON_ATTRIBUTE_SECONDARIES || [];
            return LT.WEAPON_MAJOR_SECONDARIES || [];
        }
        if (primary === "CLOTHING_ATTRIBUTE")
            return LT.CLOTHING_ATTRIBUTE_SECONDARIES;
        if (primary === "CLOTHING_SPECIAL")
            return LT.CLOTHING_SPECIAL_SECONDARIES;
        return LT.CLOTHING_MAJOR_SECONDARIES;
    }
    function chip(label, value, selected, kind) {
        return ('<button type="button" class="inv-wep-btn' +
            (selected ? " selected" : "") +
            '" data-enchant-' +
            kind +
            '="' +
    ...
    function effectHtml() {
        ...
        return ("<p>Using the essences stored in your aura, you focus on the <b>" +
            (item.name || "item") +
    ...
    LT.openEnchant = function (uid) {
        ...
        if (racial) {
            if (!s.primary || (LT.RACIAL_PRIMARIES || []).indexOf(s.primary) < 0)
                s.primary = "TF_CORE";
        }
        else if (weapon) {
            if (!s.primary || s.primary.indexOf("TF_") === 0 || s.primary === "CLOTHING_SPECIAL")
                s.primary = "CLOTHING_MAJOR_ATTRIBUTE";
        }
        else if (!s.primary || s.primary.indexOf("TF_") === 0) {
            s.primary = "CLOTHING_MAJOR_ATTRIBUTE";
        }
        if (!s.secondary || secondariesFor(s.primary).indexOf(s.secondary) < 0)
            s.secondary = secondariesFor(s.primary)[0];
        if (!s.potency)
            s.potency = "MINOR_BOOST";
    ...
    LT.defineNode({
        ...
        getResponses: function () {
            ...
            if (!item)
                craft.disable("Nothing is selected.");
            return [
    ...
    document.addEventListener("click", function (e) {
        ...
        if (primary) {
            ...
            if (allowed.indexOf(s.secondary) < 0)
                s.secondary = allowed[0];
            LT.game.setContent("enchant.main");
            return;
        }
...
//# sourceMappingURL=enchantNodes.js.map