...
(function () {
    ...
    LT.isOutdoorPlace = function (placeType) {
        if (!placeType)
            return false;
        if (ALWAYS_DANGEROUS[placeType] || STORM_STREETS[placeType])
            return true;
        if (placeType === "DOMINION_PLAZA" || placeType === "DOMINION_BOULEVARD")
            return true;
        if (placeType.indexOf("DOMINION_DEMON_HOME") === 0)
            return true;
        if (placeType.indexOf("WORLD_MAP_") === 0)
            return true;
        if (/HARPY_NESTS/.test(placeType) && placeType.indexOf("HELENA") < 0)
            return true;
        return false;
    };
    ...
    LT.ensureWeather = function () {
        ...
        if (f.weatherTimeRemaining == null)
            f.weatherTimeRemaining = 0;
    ...
    LT.dominionPlaceText = function (placeType) {
        ...
        if (!tag || typeof LT.parseFromXML !== "function")
            return "";
    ...
    LT.maybeStormEncounter = function () {
        ...
        if (!loc || loc.world !== "DOMINION")
            return;
        if (!LT.isStormStreet(loc.place) || loc.place === "DOMINION_SLAVER_ALLEY")
            return;
        var key = "storm," + loc.world + "," + loc.x + "," + loc.y;
        if (LT.game.flags.stormTileKey === key)
            return;
    ...
    document.addEventListener("lt-content", function () {
        if (typeof LT.updateMapVignette === "function")
            LT.updateMapVignette();
        if (typeof renderGrid === "function" && window.grid && grid.gridName)
            renderGrid();
    });
