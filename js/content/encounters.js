...
(function () {
    ...
    function playerTrueName() {
        ...
        if (!p)
            return "";
        if (p.names)
            return p.names.androgynous || p.names.feminine || p.names.masculine || "";
        if (p.getName)
            return p.getName() || "";
        return p.name || "";
    }
    ...
    LT.rollEncounterTable = function (entries, force) {
        ...
        for (i = 0; i < (entries || []).length; i++) {
            var e = entries[i];
            if (!e || !e.weight || (e.available && !e.available()))
                continue;
            available.push(e);
            total += e.weight;
        }
        if (!available.length || total <= 0)
            return null;
        if (!force && Math.random() * 100 >= total)
            return null;
        var tries = 0;
    ...
    LT.encounterTableIdForPlace = function (placeType) {
        if (!placeType)
            return null;
        if (STREET_PLACES[placeType])
            return "DOMINION_STREET";
        if (placeType === "DOMINION_PARK")
            return "DOMINION_PARK";
        if (HARPY_WALKWAYS[placeType])
            return "HARPY_NEST_WALKWAYS";
        return null;
    };
    ...
    function entriesForTable(tableId) {
        if (tableId === "DOMINION_STREET")
            return LT.streetEncounterEntries();
        if (tableId === "DOMINION_PARK")
            return LT.parkEncounterEntries();
        if (tableId === "HARPY_NEST_WALKWAYS")
            return LT.harpyWalkwayEntries();
        if (tableId === "HARPY_NEST_LOOK_FOR_TROUBLE")
            return LT.harpyLookForTroubleEntries();
        return [];
    }
    ...
    LT.maybePlaceEncounter = function (opts) {
        ...
        if (!tableId)
            return null;
        var key = "enc," + tileKey();
        if (!opts.force && flags().encounterTileKey === key)
            return null;
        ...
        if (!result)
            return null;
        if (!opts.noRedirect)
            flags().redirectNode = result.node;
        return result.node;
    };
    ...
    function fightResponse(npc, victory, defeat, tip) {
        return LT.ResponseCombat("Fight", tip || LT.parse("Stand up for yourself and fight [npc.name]!"), {
            ...
            onEscape: function () {
                if (typeof LT.clearAlleyMugger === "function")
                    LT.clearAlleyMugger();
            },
        });
    }
    function enslaveIfPossible(list, npc) {
        if (!npc || typeof LT.getNode !== "function" || !LT.hasNode("alley.enslave"))
            return list;
    ...
    function lootIfPossible(list, npc, back) {
        if (typeof LT.lootResponse === "function")
            list.push(LT.lootResponse(npc, back));
        return list;
    }
    LT.defineNode({
        id: "enc.storm-attack",
        ...
        getResponses: function () {
            var npc = mugger();
            if (!npc)
                return [new LT.Response("Continue", "There's nobody here.", returnPlace())];
            var pay = new LT.Response("Offer money", LT.parse("Due to the ongoing arcane storm, [npc.name] isn't interested in your money, and only wants to have sex! You'll have to either fight [npc.herHim] or give [npc.herHim] what [npc.she] wants!"), null).disable(LT.parse("Due to the ongoing arcane storm, [npc.name] isn't interested in your money, and only wants to have sex! You'll have to either fight [npc.herHim] or give [npc.herHim] what [npc.she] wants!"));
    ...
    LT.defineNode({
        ...
        getContent: function () {
            ...
            if (n && n.attractedToPlayer)
                return stormXml("AFTER_COMBAT_VICTORY_ATTRACTION");
    ...
    LT.defineNode({
        ...
        getContent: function () {
            var n = mugger();
            if (n && n.attractedToPlayer)
                return harpyXml("AFTER_COMBAT_VICTORY_ATTRACTION");
        ...
        getResponses: function () {
            ...
            return [
                ...
                new LT.Response("Take", "Add the " + name + " to your inventory.", returnPlace(), function () {
                    if (id && typeof LT.addItem === "function")
    ...
    LT.harpyExploreResponses = function () {
        ...
        if (pacified) {
            list.push(new LT.Response("Look for trouble", "Although you've pacified the harpy nests, you're sure that you can find a harpy who's looking for a confrontation...", null, function () {
            ...
            list.push(new LT.Response("Explore", "Explore the walkways. Although you don't think you're any more or less likely to find anything by doing this, at least you won't have to keep travelling back and forth...", null, function () {
                LT.game.advanceTime(1800);
