...
(function () {
    ...
    function inferTravelLabel(cfg) {
        if (!cfg)
            return "Travel";
        if (cfg.label && cfg.label !== "To")
            return cfg.label;
        var dest = cfg.nextGridName || "";
        if (/FIRST_FLOOR|_f1|_F1/i.test(dest))
            return "Upstairs";
        if (/GROUND_FLOOR|_f0|_F0/i.test(dest))
            return "Downstairs";
        if (/WORLD_MAP/i.test(dest))
            return "World map";
        return "Travel";
    }
    ...
    LT.canUseTileTravel = function (cfg) {
        if (!cfg || !cfg.nextGridName)
            return true;
        if (/WORLD_MAP/i.test(cfg.nextGridName) && !discoveredWorldMap())
            return false;
        return true;
    };
    LT.travelResponses = function () {
        ...
        if (isDominionExit(placeType)) {
            if (discoveredWorldMap()) {
                list.push(new LT.Response("World travel", "Exit Dominion and head out into the wide world...", null, function () {
                    LT.useTileTravel();
                }));
            }
            else {
                list.push(new LT.Response("World travel", "You don't know what the rest of the world looks like, and, for now, your business is within the city.", null).disable("You don't know what the rest of the world looks like, and, for now, your business is within the city."));
            }
            return list;
        }
    ...
    LT.defineNode({
        ...
        applyPreParsingEffects: function () {
            ...
            else if (typeof LT.maybeStormEncounter === "function")
                LT.maybeStormEncounter();
        ...
        getContent: function () {
            ...
            if (isDominionExit(info.placeType)) {
                if (discoveredWorldMap()) {
                    return p("A pair of elite demon Enforcers are keeping a close watch on everyone who enters or leaves the city. Now that you have a map, as well as business out there in the world beyond Dominion, there's nothing stopping you from leaving right now.");
                }
                return (p("A pair of elite demon Enforcers are keeping a close watch on everyone who enters or leaves the city. Although there's nothing stopping you from heading out into the world beyond, you have no reason to leave Dominion at the moment, and, without a map, you imagine that it would be quite easy to get lost.") +
    ...
    STREET_NODES.forEach(function (spec) {
        if (LT.hasNode && LT.hasNode("place." + spec.id))
            return;
        LT.defineNode({
            id: "place." + spec.id,
            ui: "dialogue",
