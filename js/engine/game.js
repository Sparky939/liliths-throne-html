...
(function () {
    ...
    Game.prototype.setContent = function (node) {
        ...
        if (typeof node === "string")
            node = LT.getNode(node);
        this.currentNode = node;
        if (node.applyPreParsingEffects)
            node.applyPreParsingEffects(this);
        ...
        if (app) {
            if (node.travelDisabled)
                app.classList.add("travel-disabled");
            else
                app.classList.remove("travel-disabled");
        }
        ...
        if (node.id === "sex.scene" && LT.sex && LT.sex.responseTab)
    ...
    Game.prototype.choose = function (response) {
        ...
        if (response.secondsPassed != null)
            ...
            this.advanceTime(this.currentNode.secondsPassed);
        if (response.effects)
            response.effects(this);
        if (response.nextDialogue)
            this.setContent(response.nextDialogue);
    };
    Game.prototype.advanceTime = function (seconds) {
        this.secondsPassed += seconds;
        if (typeof LT.tickWeather === "function")
            LT.tickWeather(seconds);
        if (typeof LT.tickSlavery === "function")
            LT.tickSlavery(seconds);
        ...
        if (typeof LT.tickRegeneration === "function" && this.player)
            LT.tickRegeneration(this.player, seconds);
    ...
    LT.scarlettPrice = function () {
        if (LT.game.flags && LT.game.flags.scarlettPrice)
            return LT.game.flags.scarlettPrice;
