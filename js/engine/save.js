...
(function () {
    ...
    function applyPlayer(p, data) {
        ...
        if (data.raceName)
            p.raceName = data.raceName;
        if (data.fullRace)
            p.fullRace = data.fullRace;
        if (data.appearedAge != null)
            p.appearedAge = data.appearedAge;
        if (data.penisPresent != null)
            p.penisPresent = data.penisPresent;
        if (data.vaginaPresent != null)
            p.vaginaPresent = data.vaginaPresent;
        if (typeof LT.applySavedBody === "function")
            LT.applySavedBody(p, data);
        ...
        else if (typeof LT.refreshVitals === "function")
            LT.refreshVitals(p);
        return p;
    }
    function readIndex() {
    ...
    LT.saveGame = function (name) {
        if (!LT.game || !LT.game.started || !LT.game.player)
            return false;
    ...
    LT.deleteSave = function (name) {
        ...
        writeIndex(readIndex().filter(function (n) {
            return n !== name;
        }));
    };
    ...
    LT.loadGame = function (name) {
        ...
        try {
            ...
            if (data.startingYear != null)
                LT.game.startingYear = data.startingYear;
            if (data.startingMonth != null)
                LT.game.startingMonth = data.startingMonth;
            if (data.startingDay != null)
                LT.game.startingDay = data.startingDay;
            ...
            if (typeof LT.syncSlaveNpcs === "function")
                LT.syncSlaveNpcs();
            if (nodeId && LT.hasNode(nodeId))
                LT.game.setContent(nodeId);
                ...
                if (passage && LT.hasNode(passage))
                    LT.game.setContent(passage);
    ...
    LT.rememberReturn = function () {
        ...
        if (!node || !node.id)
            return;
        if (/^(boot\.|phone\.|inventory\.|body\.)/.test(node.id))
            return;
        LT.game.returnNode = node;
    };
    LT.autoSave = function () {
        if (LT._loading || !LT.game || !LT.game.started)
            return;
        try {
