...
(function () {
    ...
    LT.worldRegionOf = function (world) {
        if (!world)
            return "";
        if (world === "MUSEUM" || world === "MUSEUM_LOST")
            return "OLD_WORLD";
        if (/HARPY/i.test(world))
            return "HARPY_NESTS";
        if (/WORLD_MAP/i.test(world))
            return "WORLD_MAP";
        if (/SUBMISSION|BAT_CAVERN/i.test(world))
            return "SUBMISSION";
        if (/FIELD|ELIS|THEMISC/i.test(world))
            return "FIELDS";
        return "DOMINION";
    };
    ...
    function weatherRemainingExtra(_ch) {
    ...
    se("WEATHER_CLEAR", {
        ...
        description: function (ch) {
            var sky = typeof LT.isDayTime === "function" && LT.isDayTime()
    ...
    se("WEATHER_CLOUD", {
        ...
        extra: weatherLibidoExtras,
        description: function (ch) {
            return ("The weather seems to change at a moment's notice, and is currently overcast, with a chance of rain. Although there's no sign of a storm at the moment, " +
                nameOf(ch) +
    ...
    se("WEATHER_RAIN", {
        ...
        extra: weatherLibidoExtras,
        description: function (ch) {
            return ("The heavy rain clouds overhead have finally burst, unleashing a sudden, torrential downpour. Although there's no sign of a storm at the moment, " +
                nameOf(ch) +
                " can still feel the effects of the arcane manifesting in the form of an increased libido.");
        },
        conditions: function (ch) {
    ...
    se("WEATHER_SNOW", {
        ...
        extra: weatherLibidoExtras,
        description: function (ch) {
            return ("The heavy clouds overhead have finally burst, unleashing a flurry of brilliant white snowflakes upon the land below. Although there's no sign of a storm at the moment, " +
                nameOf(ch) +
                " can still feel the effects of the arcane manifesting in the form of an increased libido.");
        },
        conditions: function (ch) {
    ...
    se("WEATHER_STORM_GATHERING", {
        ...
        extra: weatherLibidoExtras,
        description: function (ch) {
            return parseFor(ch, "A roiling mass of thick black storm clouds hang heavy in the skies above [npc.name]. Flashes of pink and purple energy can be seen just beneath their surface, and [npc.she] [npc.verb(realise)] that an arcane storm is going to break out at any moment.");
        },
        conditions: function (ch) {
    ...
    se("WEATHER_STORM", {
        ...
        conditions: function (ch) {
            if (!LT.isInNewWorld(ch) || typeof LT.currentWeather !== "function" || LT.currentWeather() !== "MAGIC_STORM")
                return false;
            var immune = typeof LT.isStormImmunePlace === "function" && LT.isStormImmunePlace(currentPlace(ch));
            if (!LT.isStormRegion(ch))
                return true;
    ...
    se("WEATHER_STORM_VULNERABLE", {
        ...
        conditions: function (ch) {
            ...
            if (!LT.isStormRegion(ch))
                return false;
            if (typeof LT.isStormImmunePlace === "function" && LT.isStormImmunePlace(currentPlace(ch)))
                return false;
    ...
    se("WEATHER_STORM_PROTECTED", {
        ...
        description: function (ch) {
            return ("Huge streaks of pink and purple lightning arc through the sky as an arcane storm rages high above Dominion. Although they can still feel its effects taking the form of an increased libido, " +
                nameOf(ch) +
    ...
    se("WELL_RESTED_BOOSTED_EXTRA", {
        ...
        description: function (ch) {
            return parseFor(ch, "Thanks to the upgraded emperor-size bed in [npc.her] room, combined with [npc.her] ability of knowing how best to get a good rest, [npc.name] now [npc.verb(feel)] as though [npc.sheIs] overflowing of energy and vigour.");
        },
    });
    ...
    se("RECOVERING_AURA", {
        ...
        description: function (ch) {
            if (ch && ch.player)
                return "Due to a recent orgasm, your arcane aura has been temporarily strengthened. While in this state, you will no longer receive an arcane essence if you orgasm!";
            return parseFor(ch, "Due to a recent orgasm, [npc.namePos] arcane aura has been temporarily strengthened. While [npc.she] remains in this state, you will not receive an arcane essence if [npc.she] orgasms in your presence!");
        },
    });
    se("FLASH", {
        name: "Blinded",
    ...
    se("PREGNANT_0", {
        ...
        description: function (ch) {
            return parseFor(ch, "After recently having unprotected sex, there's a risk that [npc.name] will get pregnant! Due to the fact that the arcane accelerates people's pregnancies, [npc.she]'ll know if [npc.sheIs] pregnant within a matter of hours...");
        },
    ...
    se("PREGNANT_3", {
        ...
        conditions: function (ch) {
            return !!(typeof LT.isPregnant === "function" &&
                LT.isPregnant(ch) &&
    ...
    LT.addStatusEffect = function (ch, id, duration) {
        if (!ch || !LT.STATUS_EFFECTS[id])
            return false;
        var def = LT.STATUS_EFFECTS[id];
        var rec = bag(ch)[id] || { id: id, lastApplied: nowSeconds(), secondsPassed: 0 };
        if (duration && typeof duration === "object") {
            if (duration.combatTurns != null)
                rec.combatTurns = duration.combatTurns;
            if (duration.secondsRemaining != null)
                rec.secondsRemaining = duration.secondsRemaining;
        }
        else if (def.combat) {
    ...
    LT.removeStatusEffect = function (ch, id) {
        if (!ch || !ch.statusEffects || !ch.statusEffects[id])
            return false;
    ...
    LT.listStatusEffects = function (ch) {
        ...
        Object.keys(ch.statusEffects).forEach(function (id) {
            var def = LT.STATUS_EFFECTS[id];
            if (def)
                list.push({ id: id, def: def, applied: ch.statusEffects[id] });
        });
            ...
            if (rec.secondsRemaining != null && rec.secondsRemaining >= 0) {
                ...
                if (rec.secondsRemaining < 0) {
                    if (def.onExpire) {
                        ...
                        if (msg && LT.game)
                            LT.game.textEnd = (LT.game.textEnd || "") + msg;
                    }
                    delete bagNow[id];
                }
            }
    ...
    LT.paintStatusEffects = function (ch) {
        ...
        for (i = 0; i < icons.length; i++) {
            (function (el) {
                if (typeof LT.bindTooltip === "function")
