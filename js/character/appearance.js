...
(function () {
    ...
    function hexOf(list, id, fallback) {
        ...
        if (list) {
            ...
            if (typeof LT.findById === "function") {
                var hit = LT.findById(list, id);
                if (hit)
                    return hit.hex || hit.colour || fallback || "#ddd";
            }
            if (Array.isArray(list)) {
                for (var i = 0; i < list.length; i++)
                    if (list[i].id === id)
    ...
    function heightLabel(cm) {
        var n = Number(cm) || 170;
        if (n < 152)
            return { name: "very short", colour: "#c9dde8" };
        if (n < 165)
            return { name: "short", colour: "#9ec9dc" };
        if (n < 178)
            return { name: "average height", colour: "#88b8d4" };
        if (n < 191)
            return { name: "tall", colour: "#6fa4c4" };
    ...
    function ageCategory(age) {
        ...
        for (i = 0; i < list.length; i++) {
            if (age >= list[i].min && age <= list[i].max)
                return list[i];
        }
    ...
    function bodyHairPhrase(level, area) {
        ...
        if (!level || level === "ZERO_NONE")
            return "";
        if (level === "ONE_STUBBLE")
            return "a stubbly patch of " + area;
        if (level === "TWO_MANICURED")
            return "a small, manicured amount of " + area;
        if (level === "THREE_TRIMMED")
            return "a well-trimmed amount of " + area;
        if (level === "FOUR_NATURAL")
            return "a natural amount of " + area;
        if (level === "FIVE_UNKEMPT")
            return "an unkempt amount of " + area;
        if (level === "SIX_BUSHY")
            return "a thick, bushy amount of " + area;
        if (level === "SEVEN_WILD")
            return "a wild amount of " + area;
        return n + " " + area;
    }
    LT.knowsArea = function (target, area) {
        ...
        if (area === "PENIS" || area === "VAGINA" || area === "ANUS" || area === "NIPPLES")
            return false;
        return true;
    };
    ...
    function overview(ch, v, b) {
        ...
        if (!isPlayer(ch) && LT.game && LT.game.player && LT.game.player.heightCm) {
            ...
            if (diff >= 25)
                html += " making " + v.him + " <span style='color:#4f88ab;'>significantly taller</span> than you.";
            else if (diff <= -25)
                html += " making " + v.him + " <span style='color:#c9dde8;'>significantly shorter</span> than you.";
        }
    ...
    function faceSection(ch, v, b) {
        ...
        if (blush)
            html += " " + v.SheIs + " wearing " + blush + " blusher.";
        ...
        if (LT.knowsArea(ch, "EYES")) {
            ...
            if (liner)
                html += " Around " + v.her + " eyes, " + v.she + " " + v.has + " a layer of " + liner + " eye liner.";
            var shadow = makeupColour(ch, "MAKEUP_EYE_SHADOW");
            if (shadow)
                html += " " + v.SheIs + " wearing a tasteful amount of " + shadow + " eye shadow.";
        }
        else {
    ...
    function crotchSection(ch, v, b) {
        ...
        if (!crotch || crotch.type === "NONE" || !crotch.rows)
            return "";
    ...
    function armsSection(ch, v, b) {
        ...
        if (typeof LT.hasProperty !== "function" || LT.hasProperty("bodyHairContent")) {
            ...
            if (under === "ZERO_NONE")
                ...
                html += " " + v.SheHas + " " + bodyHairPhrase(under, "underarm hair") + " in " + v.her + " armpits.";
        }
        return html + "</p>";
    }
    function legsSection(ch, v, b) {
        ...
        if (toes)
            html += " " + v.Her + " toenails have been painted " + toes + ".";
        return html + "</p>";
    }
    ...
    function assSection(ch, v, b) {
        ...
        if (!LT.knowsArea(ch, "ANUS")) {
            ...
            if (typeof LT.hasProperty !== "function" || LT.hasProperty("assHairContent")) {
                ...
                if (ah !== "ZERO_NONE")
                    html += " " + v.SheHas + " " + bodyHairPhrase(ah, "ass hair") + " around " + v.her + " asshole.";
            }
        }
        return html + "</p>";
    }
    ...
    function vaginaSection(ch, v, b) {
        ...
        if (LT.knowsArea(ch, "VAGINA")) {
            ...
            if (b.vagina && b.vagina.hymen)
                html += " " + v.Her + " hymen is intact.";
            if ((ch.piercings && ch.piercings.vagina) || (b.vagina && b.vagina.pierced))
                html += " It has been pierced.";
            if (typeof LT.hasProperty !== "function" || LT.hasProperty("pubicHairContent")) {
                var ph = b.pubicHair || "ZERO_NONE";
    ...
    function relationshipHtml(ch) {
        ...
        var aff = 0;
        if (typeof ch.getAffection === "function")
            aff = ch.getAffection();
        else if (typeof ch.affection === "number")
            aff = ch.affection;
        ...
        if (ch.owner === "player" || ch.slaveOwner === "player")
            html += "<br/>" + v.SheIs + " a <span style='color:" + LT.Colour.GENERIC_ARCANE + ";'>slave</span>, owned by you.";
        return html + "</p>";
    }
    ...
    function statsHtml(ch) {
        ...
        return ("<div class='container-full-width'><details><summary class='quest-title'>Stats</summary>" +
            "<p>Level " +
            (ch.level || 1) +
            " · Health " +
    ...
    LT.describeBody = function (p) {
        if (typeof LT.getBodyDescription === "function")
            return LT.getBodyDescription(p);
