...
(function () {
    ...
    function gridWidth() {
        if (grid.gridData && grid.gridData[0])
            return grid.gridData[0].length;
        return grid.gridWidth || grid.gridSize || 25;
    }
    function gridHeight() {
        if (grid.gridData)
            return grid.gridData.length;
    ...
    function findTile(gridData, x, y) {
        if (!gridData)
            return null;
        if (gridData[y] && gridData[y][x] && typeof gridData[y][x].x === "number")
            return gridData[y][x];
        for (var row = 0; row < gridData.length; row++) {
            if (!gridData[row] || typeof gridData[row].length !== "number")
                continue;
        ...
        for (var row = 0; row < gridData.length; row++) {
            for (var col = 0; col < gridData[row].length; col++) {
                ...
                if (tile.travelConfig != null)
                    minTile.travelConfig = tile.travelConfig;
    ...
    function getMaxifiedGrid(minimizedGrid, gridSize, gridHeight) {
        if (isAlreadyMaxified(minimizedGrid))
            return minimizedGrid;
        ...
        if (Array.isArray(flat)) {
            for (var i = 0; i < flat.length; i++) {
                ...
                if (typeof t.x === "number" && t.x + 1 > width)
                    width = t.x + 1;
                if (typeof t.y === "number" && t.y + 1 > height)
                    height = t.y + 1;
            }
        }
        var lookup = {};
    ...
    function getCurrentTile() {
        if (!grid.gridData || !grid.playerPosition)
            return null;
    ...
    function collectLocationsFromGrid(fullGrid) {
        ...
        for (var y = 0; y < fullGrid.length; y++) {
            for (var x = 0; x < fullGrid[y].length; x++) {
                ...
                if (!loc || !loc.name || seen[loc.name + "|" + (loc.placeType || "")])
                    continue;
        ...
        for (var row = startY; row < startY + viewH; row++) {
            for (var col = startX; col < startX + viewW; col++) {
                ...
                var bg = (knownVisual && visual && visual.background)
                    ...
                    || "#bbbbbb";
                if (tile.isNavigable && bg)
                    tileDiv.style.backgroundColor = bg;
    ...
    function movePlayer(dx, dy, moveMode) {
        ...
        function runTravelHandler() {
            ...
            renderGrid();
            updateInfo();
            if (typeof grid.onMove === "function")
                grid.onMove(getCurrentTile(), grid);
        }
        if (moveMode === "Default") {
    ...
    function loadGrid(newGrid, tile) {
        ...
        renderGrid();
        updateInfo();
        if (typeof grid.onLoad === "function")
            grid.onLoad(getCurrentTile(), grid);
    }
    ...
    function generateGrid(mode) {
        ...
        if (typeof window.generateDungeon === "function" && grid.gridStyle && grid.gridStyle !== "empty") {
            ...
            if (style === "corridors" || style === "corridors_rooms" || style === "thick_corridors" || style === "better_rooms") {
                ...
                if (style === "corridors_rooms" && typeof window.addRooms === "function")
                    window.addRooms(newGrid);
                if (style === "thick_corridors" && typeof window.thickenCorridors === "function")
                    window.thickenCorridors(newGrid, 2);
                if (style === "better_rooms" && typeof window.addBetterRooms === "function")
                    window.addBetterRooms(newGrid);
            }
            ...
            grid.gridData = Array.from({ length: size }, function (_, gy) {
                return Array.from({ length: size }, function (_, gx) {
                    ...
                    var location = null;
                    if (value === 1)
                        location = { color: "#bbb", name: "Corridor" };
                    else if (value === 2)
                        location = { color: "#888", name: "Room" };
        ...
        if (firstTile)
            grid.playerPosition = grid.playerPosition || { x: firstTile.x, y: firstTile.y };
    ...
    function travelLocked() {
        if (window.LT && LT.game && LT.game.currentNode && LT.game.currentNode.travelDisabled)
            return true;
        if (window.LT && LT.combat && LT.combat.active)
            return true;
        if (window.LT && LT.sex && LT.sex.active)
            return true;
        return false;
    }
    function selectTile(row, col) {
        if (travelLocked())
            return;
        if (!grid.gridData || !grid.gridData[row] || !grid.gridData[row][col])
            return;
        ...
        if (Array.isArray(currentGrid) && currentGrid.length && currentGrid[0] && typeof currentGrid[0].x === "number") {
            ...
            for (var y = 0; y < currentGrid.length; y++) {
                for (var x = 0; x < currentGrid[y].length; x++) {
                    ...
                    if (t && t.location && t.location.name === locationName)
                        matches.push(t);
                }
            }
        }
