...
(function () {
    ...
    LT.shopItemIds = function (seller) {
        ...
        for (id in ITEMS) {
            if (!Object.prototype.hasOwnProperty.call(ITEMS, id))
                continue;
            if ((ITEMS[id].soldBy || []).indexOf(seller) >= 0)
                ids.push(id);
        }
        return ids;
    };
    ...
    LT.buyItem = function (player, id) {
        var type = ITEMS[id];
        if (!type)
            return "<p>That is not for sale.</p>";
        ...
        if (typeof LT.incrementMoney === "function")
        ...
        for (i = 0; i < ids.length; i++) {
            (function (id) {
                ...
                if (typeof LT.getMoney === "function" && LT.getMoney() < price) {
                    ...
                    list.push(new LT.Response(title, "Buy " + type.name + " for " + price + " flames.", null, function () {
