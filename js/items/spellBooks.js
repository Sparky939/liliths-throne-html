...
(function () {
    ...
    LT.vickyBookStock = function () {
        ...
        if (LT.game.flags.vickyBookDay === day && LT.game.flags.vickyBooks)
            return LT.game.flags.vickyBooks;
        var stock = {};
        for (var i = 0; i < LT.SPELL_BOOK_IDS.length; i++)
            stock[LT.SPELL_BOOK_IDS[i]] = 1;
        LT.game.flags.vickyBookDay = day;
        LT.game.flags.vickyBooks = stock;
        return stock;
    };
    LT.readSpellBook = function (player, book) {
        if (!player || !book || !book.spellId)
            return "That is not a spell book.";
        var spell = LT.SPELLS[book.spellId];
        if (!spell)
            return "That spell is not in this build.";
        var known = player.knownSpells || [];
        if (known.indexOf(book.spellId) >= 0) {
            return ("<p>Reading through the spell book again, you quickly discover that you've already learned all there is to know about the spell '" +
                spell.name +
        ...
        if (typeof LT.learnSpell === "function")
            LT.learnSpell(player, book.spellId);
        player.items = (player.items || []).filter(function (it) {
            return it !== book && it.uid !== book.uid;
        });
        return ("<p>Opening the spell book, you read its contents...</p>" +
...
//# sourceMappingURL=spellBooks.js.map