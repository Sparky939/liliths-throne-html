...
(function () {
    ...
    function stepEnum(current, list, amount) {
        ...
        for (i = 0; i < list.length; i++) {
            if (current && list[i] && (list[i] === current || list[i].id === current.id))
                break;
        }
        if (i >= list.length)
            i = 0;
    ...
    function applyRaceCovering(ch, item) {
        ...
        if (ch.body) {
            ...
            if (race === "DEMON")
                ch.body.raceStage = "GREATER";
        }
        return "Your body takes on more " + t.race + " features.";
    }
    function syncParts(ch) {
        if (ch.penisPresent == null)
            ch.penisPresent = !!(ch.gender && ch.gender.hasPenis);
        if (ch.vaginaPresent == null)
            ch.vaginaPresent = !!(ch.gender && ch.gender.hasVagina);
    }
    function setPenis(ch, on) {
        syncParts(ch);
        ch.penisPresent = !!on;
        if (on && !ch.penisLength)
            ch.penisLength = 15;
        if (ch.body && ch.body.penis) {
            ch.body.penis.type = on ? ch.body.penis.type === "NONE" ? "HUMAN" : ch.body.penis.type : "NONE";
            if (on && !ch.body.penis.length)
                ch.body.penis.length = ch.penisLength || 15;
        }
    }
    ...
    LT.isRacialIngredient = function (item) {
        if (!item)
            return false;
        if (item.kind === "tf" || item.kind === "potion")
            return true;
        var t = drinkType(item);
        return !!(t && t.kind === "tf");
    };
    LT.applyRacialEffect = function (ch, effect, item) {
        ...
        if (sec === "REMOVAL") {
            if (pri === "TF_PENIS") {
                if (!ch.hasPenis())
                    return "You have no penis to remove.";
                setPenis(ch, false);
                return "Your penis shrinks away until nothing remains.";
            }
            if (pri === "TF_VAGINA") {
                if (!ch.hasVagina())
                    return "You have no vagina to remove.";
                setVagina(ch, false);
        ...
        if (pri === "TF_CORE") {
            if (sec === "TF_MOD_SIZE") {
                ...
                if (ch.heightCm === before)
                    return "Your height does not change.";
...
//# sourceMappingURL=tfApply.js.map