...
(function () {
    ...
    LT.itemEffectCost = function (effect) {
        ...
        if (p && effect.primary !== "NONE")
            cost += p.value;
        if (s && effect.secondary !== "NONE")
            cost += s.value;
        if (pot)
            cost += pot.value;
        if (effect.limit !== -1 && effect.limit != null)
            cost += 1;
        if (effect.secondary === "CLOTHING_SEALING" && pot) {
            if (effect.potency === "MAJOR_BOOST")
                cost *= 4;
            else if (effect.potency === "BOOST")
                cost *= 2;
        }
        return cost;
    };
    LT.enchantCost = function (ingredient, effects) {
        ...
        for (var i = 0; i < incoming.length; i++)
            cost += LT.itemEffectCost(incoming[i]);
    ...
    LT.effectLabel = function (effect) {
        ...
        if (effect.secondary === "CLOTHING_SEALING")
            return pot.name + " sealing";
        if (sec.attr)
            return pot.name + " " + sec.name + " (" + (pot.clothingBonus > 0 ? "+" : "") + pot.clothingBonus + ")";
        if (effect.type === "RACIAL" || (pri.id && pri.id.indexOf("TF_") === 0)) {
            if (effect.secondary === "TF_TYPE_1")
                return pri.name + " — racial type";
            if (effect.secondary === "REMOVAL")
                return "remove " + pri.name;
    ...
    LT.reapplyWornEnchantments = function (ch) {
        ...
        if (typeof LT.getOffhandWeapon === "function")
            addWeapon(LT.getOffhandWeapon(ch));
        else
            addWeapon(ch.offhandWeapon);
    ...
    LT.incrementEssenceCount = function (amount, withText) {
        ...
        return ("<p style='text-align:center;'>You have <b style='color:" +
            colour +
            ";'>" +
            verb +
            " " +
    ...
    LT.awardCombatEssences = function (enemy) {
        ...
        if (typeof LT.stormDoublesEssences === "function" && LT.stormDoublesEssences())
            gained *= 2;
        var html = "";
        var f = LT.game.flags;
    ...
    LT.awardOrgasmEssences = function () {
        var p = LT.game && LT.game.player;
        if (p && typeof LT.hasStatusEffect === "function" && LT.hasStatusEffect(p, "RECOVERING_AURA"))
            return "";
    ...
    LT.craftEnchantedItem = function (ingredient, effects) {
        if (!ingredient)
            return { error: "Nothing to enchant." };
        var next = effects || [];
        ...
        if (racial)
            crafted.kind = "tf";
        if (crafted.name && crafted.name.indexOf("enchanted ") !== 0)
            crafted.name = "enchanted " + crafted.name;
