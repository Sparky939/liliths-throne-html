...
(function () {
    ...
    function ensureLists(ch) {
        ...
        if (!ch.weapons)
            ch.weapons = [];
        if (ch.mainWeapon === undefined)
            ch.mainWeapon = null;
        if (ch.offhandWeapon === undefined)
    ...
    LT.makeWeapon = function (id, damageType) {
        ...
        if (!chosen || type.damageTypes.indexOf(chosen) < 0)
            chosen = type.damageTypes[0];
        return {
            kind: "weapon",
            id: type.id,
            name: type.name,
    ...
    function varianceOf(weapon) {
        ...
        if (key && LT.DAMAGE_VARIANCE[key] != null)
            return LT.DAMAGE_VARIANCE[key];
    ...
    LT.equipOfficialLoadout = function (who, ch) {
        if (!ch || typeof LT.makeWeapon !== "function")
            return ch;
        ensureLists(ch);
        if (who === "brax") {
    ...
    LT.armMuggerFromOutfit = function (ch, opts) {
        ...
        if (opts.dark) {
            ...
            if (dagger) {
                ...
                if (dual)
                    ch.offhandWeapon = LT.makeWeapon("innoxia_dagger_dagger", elem());
            }
            else if (level >= 8) {
                ch.mainWeapon = LT.makeWeapon("innoxia_crystal_epic", elem());
                if (dual)
                    ch.offhandWeapon = LT.makeWeapon("innoxia_feather_epic", elem());
            }
            else {
                ch.mainWeapon = LT.makeWeapon("innoxia_crystal_rare", elem());
                if (dual)
                    ch.offhandWeapon = LT.makeWeapon("innoxia_feather_rare", elem());
            }
            return ch;
        }
        if (opts.hasWeapon === false || (opts.hasWeapon == null && rnd() > 0.9))
            return ch;
    ...
    LT.equipWeapon = function (player, uid, slot) {
        ...
        if (!type)
            return false;
        if (slot === "offhand" && type.twoHanded)
            return false;
        player.weapons.splice(idx, 1);
        if (slot === "offhand") {
            if (LT.isTwoHandedEquipped(player))
    ...
    LT.ownedWeaponIds = function (player) {
        ...
        var ids = {};
        if (player.mainWeapon)
            ids[player.mainWeapon.id] = true;
        if (player.offhandWeapon)
            ids[player.offhandWeapon.id] = true;
        for (var i = 0; i < player.weapons.length; i++)
            ids[player.weapons[i].id] = true;
        return ids;
    };
    ...
    LT.vickyWeaponIds = function () {
        ...
        for (var i = 0; i < ids.length; i++) {
            ...
            if (!type || !type.tags)
                continue;
            if (type.tags.indexOf("SOLD_BY_VICKY") < 0)
                continue;
            if (type.tags.indexOf("SILLY_MODE") >= 0)
                continue;
            out.push(ids[i]);
        }
        return out;
    ...
    LT.vickyStock = function () {
        ...
        if (LT.game.flags.vickyStockDay === day && LT.game.flags.vickyStock)
            return LT.game.flags.vickyStock;
        var stock = {};
        var ids = LT.vickyWeaponIds();
        for (var i = 0; i < ids.length; i++)
            stock[ids[i]] = 2 + Math.floor(Math.random() * 5);
    ...
    LT.applyCrit = function (ch, moveId, turnIndex, dmg) {
        if (!LT.isMoveCrit(ch, moveId, turnIndex))
    ...
    LT.recoverThrownAfterTurn = function () {
        ...
        for (var i = 0; i < list.length; i++) {
            ...
            if (Math.random() * 100 <= chance) {
                if (rec.slot === "offhand" && !rec.ch.offhandWeapon)
                    rec.ch.offhandWeapon = rec.weapon;
                else if (rec.slot === "main" && !rec.ch.mainWeapon)
                    rec.ch.mainWeapon = rec.weapon;
        ...
        for (var i = 0; i < list.length; i++) {
            ...
            if (Math.random() * 100 <= LT.oneShotRecover(rec.weapon).combat) {
                if (rec.slot === "offhand" && !rec.ch.offhandWeapon)
                    ...
                    rec.ch.mainWeapon = rec.weapon;
                else if (rec.ch.weapons)
                    rec.ch.weapons.push(rec.weapon);
            }
        }
        if (LT.combat) {
            LT.combat.thrownThisTurn = [];
...
//# sourceMappingURL=weaponRuntime.js.map