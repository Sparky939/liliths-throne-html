(function () {
  ...
  class ClothingInstance extends (LT.Item as unknown as ItemConstructor) implements ClothingItem {
    slot: string;
    colour: string;
    colourName: string;
    covers: string[];

    constructor(id: string, name: string, slot: string, colour: string, colourName: string, covers?: string[]) {
      super({ id: id, name: name, kind: "clothing", value: 0, description: "" } as ItemCatalogEntry);
      this.slot = slot;
      this.colour = colour;
